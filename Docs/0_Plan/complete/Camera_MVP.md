...
本文件是 `Project Ark` 关于 **探索态镜头改造（Camera MVP）** 的已完成专项归档。
...
- **状态**：已完成（验收通过，2026-04-15）
- **已完成内容**：C0 冻结体验与 authority、C1 解除探索态默认 confine、C2 保留局部导演能力最小闭环、C3 同步 authoring 默认行为、C4 代表房间试玩验收
- **完成判定**：探索态镜头已从“房间边界优先”切换到“飞船尽量居中优先”，并完成代表房间验收，当前结论可作为现役口径继续参考
- **后续入口**：如继续推进前向 lead、兴趣点与更细粒度 framing，应新建后续专项，而不是继续把新工作堆回本文件
