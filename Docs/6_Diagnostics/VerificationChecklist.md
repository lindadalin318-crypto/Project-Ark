...
**版本：** v3.0  
**对齐依据：** `Docs/5_ImplementationLog/ImplementationLog.md` 最新 Level 记录（截至 `2026-04-01 23:06`）  
...
- [ ] **A4** `Level Architect` 窗口包含 `Design / Build / Validate` 三个 Tab
- [ ] **A5** 当前搭建入口明确：
...
5. 确认窗口中有 `Design / Build / Validate` 三个 Tab
...
1. 在 `Design` Tab 点击 `🌐 Open LevelDesigner.html`
2. 在浏览器中规划房间和连接
3. 点击 `💾 Export File`
4. 回到 Unity，点击 `📂 Import LevelDesigner JSON...`
