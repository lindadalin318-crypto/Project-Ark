...
>
> 参考分析见 `Docs/7_Reference/GameAnalysis/Level_Architecture_Synthesis_Minishoot_Silksong_TUNIC.md`。
...
- `Docs/7_Reference/GameAnalysis/Level_Architecture_Synthesis_Minishoot_Silksong_TUNIC.md`
- `Implement_rules.md`
...
│ CameraTrigger + BiomeTrigger + HiddenAreaMask              │
│ WorldClock → WorldPhaseManager → ScheduledBehaviour        │
│ WorldProgressManager → WorldEventTrigger / 持久世界变化     │
└─────────────────────────────────────────────────────────────┘
```
...
| Layer 4 房间容器 | Scene 中 `Room` + `RoomSO` | `RoomFactory` + 手工精修 | `RoomManager`、`RoomCameraConfiner`、`MinimapManager` | `Room` 管空间，`RoomSO` 管元数据，房间边界桥接由 `RoomCameraConfiner` 负责 |
| Layer 3 连接语义 | `Door._gateID` / `_connectionType` / `_ceremony` | Authoring | Overlay、过渡演出、进度系统 | 语义层不替代目标引用 |
| Layer 2 节奏内容 | 场景子节点 + 运行时组件 | Authoring + 运行时系统 | `Room`、`RoomManager`、`ArenaController`、`SaveBridge` | 房间战斗由 `Room` 单入口驱动，互动件状态由 `RoomFlagRegistry` 管理 |
| Layer 1 导演引导 | 场景触发器 + SO + 世界状态 | Authoring + 世界管理器 | `CameraDirector`、`AmbienceController`、`ScheduledBehaviour`、`WorldEventTrigger`、`ActivationGroup` | `Phase` 链与 `Stage` 链并行存在，不合并为单一“大导演”真相源 |
...
```text
Room_[ID]   (`ArenaController` 挂在 Room 根组件上，仅 `Arena` / `Boss` 使用)
├── Navigation
...
│   ├── SpawnPoints          (可选)
│   ├── EnemySpawner         (战斗房常见)
│   └── OpenEncounterTrigger (开放遭遇房可选)
├── Hazards
├── Decoration
├── Triggers
└── CameraConfiner        (可选，仅 HardConfine 房间)
...
| 子节点 | 内容 | 查询者 | 激活策略 |
|--------|------|--------|---------|
| `Navigation` | Door、进出点、层间导航点、静态几何宿主 | `Room`、`RoomManager`、`DoorTransitionController`、`MinimapManager`、几何 authoring 工具链 | 始终激活 |
| `Elements` | 锁、拾取物、互动件、可破坏物、NPC | `Room`、`RoomFlagRegistry` | 按房间激活 + 持久状态 |
| `Encounters` | 遭遇触发器、波次生成、Spawner 骨架 | `Room`、`RoomManager` | 按房间激活 |
...
| `CameraConfiner` | 镜头边界 | `Room`、`RoomCameraConfiner` | 始终激活 |

### 6.3 Authoring 约束

- 所有可通行门默认放在 `Navigation/Doors`
- `Navigation/Geometry` 是静态几何 authoring 宿主；根节点保持 marker-only，仅挂 `RoomGeometryRoot`
- 静态墙 Tilemap 统一 author 在 `Navigation/Geometry/OuterWalls` 与 `Navigation/Geometry/InnerWalls` 下，并使用 `Tilemap + TilemapCollider2D (+ Static Rigidbody2D + CompositeCollider2D)` 组合
- 导航生成点默认放在 `Navigation/SpawnPoints`
- 战斗生成点优先放在 `Encounters/SpawnPoints`
- `Door` 属于显式门型通路，authoring 时必须与静态墙开口协作；不要把 `Door` 直接塞进填充墙 Tilemap 内部
- 非 `Door` 型跨房间转移（例如 trap / fall / teleport，当前占位样例为 `NarrativeFallTrigger`）默认不要求墙体开口，也不自动继承 `Door` 的连接拓扑语义；只有明确建模为门型通路时才归入 `Path`
...
4. **只在需要时扩 `Room` 的收集链**：当前 `Room` 只主动收集 `Door`、`EnemySpawner`、`ArenaController`、`OpenEncounterTrigger`、`DestroyableObject` 和 SpawnPoint。只有当新元素需要被 `Room` / `RoomManager` 查询、重置或统一调度时，才允许扩展 `CollectSceneReferences()`；否则应保持组件自治。
5. **按主触发机制决定挂点**：导航事实进 `Navigation`，互动件进 `Elements`，战斗编排进 `Encounters`，环境伤害进 `Hazards`，必须在玩家进房前就绪的感知/导演类对象进 `Triggers`。
6. **编辑期 schema 按需扩展**：只有当新元素需要参与 `Level Architect` overlay、validator 或批量 authoring 工具时，才扩展编辑期 schema；纯场景手工 authoring 的运行时细节，不应反向发明脱离 Scene 主链的中间模型。

### 6.5 现役房间元素分类（六大玩法家族 + 基础设施件）

| 中文分类 | 英文标签 | 历史旧术语（兼容） | 代表类型 | 默认挂点 | 运行时 authority | 状态通道 | Validator 关注点 |
|---------|----------|--------------------|----------|----------|------------------|----------|------------------|
| **通路件** | `Path` | `Door / Gate` | `Door`、层间过渡门、时间门（仅限显式门型通路） | `Navigation` | `Door._targetRoom` / `_targetSpawnPoint` + 语义字段 | `Door` 本体 + 世界阶段 / 进度系统 | 双向连接、出生点、`_playerLayer`、门控字段、几何开口协作 |
...
| **状态件** | `Stateful` | `Persistent Room Element` | `DestroyableObject`、`BreakableWall`（基于 `DestroyableObject` 的语义标准件）、房间内永久机关、一次性揭示物 | `Elements` | 场景组件 + `RoomFlagRegistry` flag | `RoomFlagRegistry` ↔ `SaveBridge` | 父 `Room`、flag key、持久状态一致性 |
...
| **导演件** | `Directing` | `Trigger / Director` | `BiomeTrigger`、`HiddenAreaMask`、`ScheduledBehaviour`、`ActivationGroup`、`WorldEventTrigger`、`CameraTrigger`、`NarrativeFallTrigger` | `Triggers`（或显式扩展内容层） | 组件自身 / `CameraDirector` / `AmbienceController` / `RoomManager` / 世界状态事件 | 会话态 / 世界阶段态 / 世界进度态 | Trigger、phase/stage 配置、target 引用、manager 依赖 |
| **基础设施件** | `Infrastructure` | 无 | `SpawnPoint`、`CameraConfiner`（可选） | `Navigation/SpawnPoints`、`CameraConfiner` | `Room`、`Door`、`RoomCameraConfiner` 等房间运行链 | 跟随房间运行链，不作为独立玩法状态件 | 位置语义、引用完整性、`CameraConfiner` 物理设置 |
...
- `BiomeTrigger` 通过 `AmbienceController` / 全局 `Volume` 做局部氛围 override，退出后恢复全局 ambience。
- `ScheduledBehaviour` 监听 `LevelEvents.OnPhaseChanged`，按 `WorldPhaseManager` 当前 phase 启停目标对象。
- `ActivationGroup` 监听 `RoomManager.OnCurrentRoomChanged`，按是否处于所属房间做成组激活。
- `WorldEventTrigger` 监听 `LevelEvents.OnWorldStageChanged`，在达到指定 `WorldStage` 后执行**不可逆**变化，并通过存档 flag 持久化。
- `CameraTrigger` 把局部镜头导演请求压入 `CameraDirector` 的 trigger 栈；它属于导演链，但当前 **`LevelValidator` 尚未覆盖同等级的 preferred-root 校验**，authoring 需要额外人工复核。
- `NarrativeFallTrigger` 这类“触发后跨房间掉落 / 传送”的组件当前按 `Directing` / `Triggers` 口径处理；它们可以改变玩家所在房间，但不要求复用 `Door` 的双向连接、门洞几何或 minimap 邻接语义。
...
Room / RoomSO
    │
    ├── RoomManager 管理房间进入、激活与清房确权
    ├── RoomCameraConfiner 读取 ConfinerBounds 更新镜头边界
    └── Encounter / Hazard / Trigger 在房间语境内运作

RoomManager.OnCurrentRoomChanged
    │
    └── ActivationGroup / RoomCameraConfiner 等局部监听者

CameraTrigger
    │
    └── CameraDirector 维护 trigger 栈并应用局部镜头参数
...
| `LevelEvents.OnRoomFlagChanged` | Door / Stateful 组件 / Save / UI | Core 静态事件 |
| `RoomManager.OnCurrentRoomChanged` | `RoomCameraConfiner`、`ActivationGroup` 等本地监听者 | `RoomManager` 实例事件 |
| `RoomManager.NotifyRoomCleared(...)` | 房间战斗链 | `RoomManager` 统一确权 API |
...
| 工具 | 职责 | 不负责 |
|------|------|--------|
| `LevelArchitectWindow` | `Build / Quick Edit / Validate` 统一入口；在 `Build` 内提供白盒 authoring、Validation Slice 与 `Quick Play` | 不承担运行时 authority |
| `RoomFactory` | 创建标准房间根节点、导航节点、相机边界与战斗骨架 | 不替代已有场景的语义设计 |
| `DoorWiringService` | Door 双向目标引用连线与 ceremony 同步 | 不规划节奏语义 |
| `LevelValidator` | 结构、引用、门连接、战斗房配置与部分导演件挂点校验 | 不替代完整试玩 |
...
- `LevelValidator` 已覆盖 `OpenEncounterTrigger`、`HiddenAreaMask`、`BreakableWall`、`BiomeTrigger`、`ScheduledBehaviour`、`ActivationGroup`、`WorldEventTrigger` 的引用与 preferred-root 校验。
- `LevelRuntimeAssistFactory` 当前已提供 `Checkpoint`、`BreakableWall`、`OpenEncounterTrigger`、`BiomeTrigger`、`ScheduledBehaviour`、`WorldEventTrigger` 与 `Hazard` 家族 starter，用于快速起标准 authoring 骨架；运行时 authority 仍归各自组件。
- `Level Architect` 当前已将 `OuterWalls` / `InnerWalls` 画布与 `BreakableWall` starter 收口到统一的 `Wall Authoring` 模块；authoring 入口统一，但运行时默认挂点仍分别属于 `Navigation/Geometry` 与 `Elements`。
- `CameraTrigger` 当前**尚未进入同等级的 validator 收口**；凡是调整镜头导演挂点、player layer 或局部相机逻辑后，都必须补一次人工层级检查与 Play Mode 走查。
...
| `Docs/3_WorkflowsAndRules/LevelArchitect/Level_WorkflowSpec.md` | 关卡搭建与验证的操作手册 |
| `Docs/7_Reference/GameAnalysis/Level_Architecture_Synthesis_Minishoot_Silksong_TUNIC.md` | 参考游戏分析与设计输入 |
| `Docs/4_GameData/` | 区域拓扑设计稿与 JSON 输入 |
