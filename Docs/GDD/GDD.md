...
# 🤖 系统策划案：敌人 AI 基础 (Enemy AI Foundation)（🔧 实现中）

**文档版本**：v2.0 (M1-W3 最终定稿)
**优先级**：P0 (核心战斗循环)
**阅读对象**：主程序、系统程序、关卡策划
**前置依赖**：射击系统核心框架（✅ 已完成）、星图编织系统（✅ 已完成）

### 实现进度总览

| 模块 | 内容 | 状态 |
|------|------|------|
| Phase 1 核心框架 | HFSM 状态机 + IState 接口 | ✅ 已完成 |
| Phase 1 躯壳层 | EnemyEntity (IDamageable + IPoolable + 移动/HP/闪白/击退/死亡) | ✅ 已完成 |
| Phase 1 大脑层 | EnemyBrain + 外层状态 (Idle/Chase/Engage/Return) | ✅ 已完成 |
| Phase 1 攻击层 | 内层子状态机 (Telegraph→Attack→Recovery) | ✅ 已完成 |
| Phase 1 感知系统 | EnemyPerception (视觉锥 5Hz + 听觉事件 + 记忆衰减) | ✅ 已完成 |
| Phase 1 数据层 | EnemyStatsSO (近战 + 远程 + 感知 + 抗性 + 标签) | ✅ 已完成 |
| Phase 1 伤害集成 | IDamageable 接口 + Projectile/LaserBeam/EchoWave 桥接 | ✅ 已完成 |
| Phase 1 玩家受伤 | ShipHealth : IDamageable + 击退/闪白/死亡 | ✅ 已完成 |
| Phase 1 原型：莽夫 | Rusher (EnemyBrain + 近战冲锋) | ✅ 已完成 |
...
> **已实现：** `IState` 接口 + `StateMachine` 纯 C# 类，支持 Initialize/Tick/TransitionTo 和嵌套子状态机。外层四状态 (Idle/Chase/Engage/Return) + 内层三阶段 (Telegraph/Attack/Recovery) 完整工作。射手型额外扩展了 ShootState + RetreatState。

```
外层状态机（战术层）
┌─────────────────────────────────────────────┐
│  Idle ──→ Chase ──→ Engage ──→ Return       │  ← ✅ 全部实现
└─────────────────────────────────────────────┘
                        │
                        ▼
              内层状态机（攻击层）
              ┌──────────────────────────────┐
              │ Telegraph → Attack → Recovery │  ← ✅ 全部实现
              └──────────────────────────────┘
```
...
## 2. 三层架构总览 (Body / Brain / Director) — ✅ Phase 1 已完成
...
### 2.1 🏛️ 第一层：躯壳 (The Body) —— `EnemyEntity` `✅ 已完成`

> **已实现：** `EnemyEntity : MonoBehaviour, IDamageable, IPoolable`。`EnemySpawner` 通过对象池管理敌人生成/回收（Round-robin 刷怪点 + 死亡自动重生）。`ShipHealth : IDamageable` 让玩家飞船也能被敌人攻击。

物理实体层。**不思考，只执行指令。**

- **移动执行**：接收方向向量，驱动 Rigidbody2D 速度 ✅ `MoveTo()` / `StopMovement()`
- **生命管理**：HP 扣除、死亡判定 ✅ | 韧性 (Poise) 字段已定义，运行时逻辑 🟡 待实现
- **受击反馈**：闪白脉冲 ✅、击退力 ✅ | 顿帧 (HitStop) 🟡 待实现
- **伤害接口**：实现 `IDamageable` 接口 ✅，四种攻击源（Projectile/LaserBeam/EchoWave/AttackSubState）均已桥接
- **对象池**：实现 `IPoolable` 接口 ✅，`EnemySpawner` 通过 `PoolManager` 管理生成/回收
- **数据驱动**：所有数值从 `EnemyStatsSO` (ScriptableObject) 读取 ✅

### 2.2 🧠 第二层：大脑 (The Brain) —— `EnemyBrain` + HFSM `✅ 已完成`

> **已实现：** `EnemyBrain` 基类构建莽夫型 HFSM (Idle/Chase/Engage/Return)。`ShooterBrain` 继承并扩展远程射击状态图 (ShootState/RetreatState)。不同原型通过继承 + SO 参数差异化实现。Editor 模式下 OnGUI 显示当前状态名便于调试。
...
**射手型扩展状态流：** ✅
```
[Chase] ──进入 PreferredRange──→ [Shoot] (Telegraph→Burst→Recovery)
                                    │ 玩家过近 (< RetreatRange)
                                    ▼
...
## 3. 攻击设计："信号-窗口"模型 (Signal-Window Model) — ✅ 已完成

> **已实现：** `TelegraphSubState`（停止移动 + sprite 变红警告）→ `AttackSubState`（OverlapCircle 伤害区域 + Commitment 不可转向）→ `RecoverySubState`（硬直 + 标记循环完成）。射手型扩展为 Telegraph→Burst Fire（连射）→Recovery。所有阶段时长通过 `EnemyStatsSO` 配置。
...
| 阶段 | 时长 (SO 可配) | 敌人行为 | 玩家策略 | 状态 |
|------|--------------|---------|---------|------|
| **Telegraph (前摇)** | 0.3s ~ 1.0s | 停止移动，播放蓄力/变红/吼叫动画 | 识别攻击类型，准备闪避 | ✅ `TelegraphSubState` |
| **Attack (出招)** | 0.1s ~ 0.5s | 生成伤害 Hitbox/发射子弹，**不可转向 (Commitment)** | 利用走位躲避，或格挡 | ✅ `AttackSubState` (近战) / `ShootState` (远程) |
| **Recovery (恢复)** | 0.5s ~ 1.5s | 硬直状态，不移动不攻击 | **反击窗口**，玩家输出最大化 | ✅ `RecoverySubState` |
...
## 4. 感知系统 (Perception System)："听觉 > 视觉" — ✅ 已完成

> **已实现：** `EnemyPerception` MonoBehaviour，完整实现视觉 + 听觉 + 记忆三层感知。暴露 `HasTarget`、`CanSeePlayer`、`HasHeardPlayer`、`LastKnownPlayerPosition`、`DistanceToTarget` 属性供状态机查询。支持 `ResetPerception()` 对象池重置。
...
```
感知层级（按优先级）：
1. 视觉锥（LoS + 扇形检测）→ 基础巡逻发现        ✅ PerformVisionCheck() 5Hz
2. 听觉范围（圆形，受武器噪音值影响）→ 射击暴露位置  ✅ HandleWeaponFired() 事件订阅
3. 记忆衰减（LastKnownPosition + 超时遗忘）→ 脱战回归  ✅ UpdateMemoryDecay()
```

### 4.2 技术实现原则 `✅ 全部遵循`

- **视觉**：定频检测（每 0.2 秒一次），不每帧执行 `Physics2D.OverlapCircle` ✅
- **听觉**：**事件订阅 + 距离校验**。监听 `StarChartController.OnWeaponFired` 静态事件，通过距离判断是否在 `HearingRange` 内 ✅
- **记忆**：失去视线后维持 `LastKnownPlayerPosition` 一段时间（`MemoryDuration`），超时后清除，敌人脱战回归 ✅

### 4.3 策略空间

- 玩家使用低噪音武器（如近战/消音）可以避免惊动远处的敌人 ✅ 基础设施就绪（噪音半径参数化）
- 玩家可以利用掩体/视觉盲区绕过敌人 ✅ LoS Raycast 支持遮挡
- 未来可支持"噪音诱饵"道具（扔到远处吸引敌人）🟡 待实现

---

## 5. 数据驱动配置 (EnemyStatsSO) — ✅ 已完成

> **已实现：** `EnemyStatsSO : ScriptableObject`，覆盖身份/生命/移动/近战/远程/感知/脱战/表现/抗性/标签共 30+ 个字段。`BestiaryImporter` Editor 工具支持从 `Enemy_Bestiary.csv`（50 列）一键批量导入/更新 SO 资产。当前 CSV 中已配置 26 种敌人完整数据。
...
| **生命** | Poise | float | 韧性值（削减到0触发硬直） | ✅ 字段定义，运行时逻辑待实现 |
| **移动** | MoveSpeed | float | 移动速度 | ✅ |
| **近战攻击** | AttackDamage | float | 攻击伤害 | ✅ |
| **近战攻击** | AttackRange | float | 攻击距离 | ✅ |
| **近战攻击** | AttackCooldown | float | 攻击间隔 | ✅ |
| **攻击阶段** | TelegraphDuration | float | 前摇时长 | ✅ |
| **攻击阶段** | AttackActiveDuration | float | 出招时长 | ✅ |
| **攻击阶段** | RecoveryDuration | float | 恢复时长 | ✅ |
| **远程攻击** | ProjectilePrefab | GameObject | 远程弹头预制体 | ✅ *新增* |
| **远程攻击** | ProjectileSpeed | float | 弹头飞行速度 | ✅ *新增* |
| **远程攻击** | ProjectileDamage | float | 弹头伤害 | ✅ *新增* |
| **远程攻击** | ShotsPerBurst | int | 连射弹数 | ✅ *新增* |
| **远程攻击** | BurstInterval | float | 连射间隔 | ✅ *新增* |
| **远程攻击** | PreferredRange | float | 理想射击距离 | ✅ *新增* |
| **远程攻击** | RetreatRange | float | 后撤触发距离 | ✅ *新增* |
...
| **抗性** | Resist_Physical~Void | float×5 | 五种元素抗性 (0~1) | ✅ *新增* |
| **掉落** | DropTableID | string | 掉落表 ID | ✅ *新增* |
| **元数据** | PlanetID | string | 归属星球 | ✅ *新增* |
| **元数据** | SpawnWeight | float | 生成权重 | ✅ *新增* |
| **元数据** | BehaviorTags | List\<string\> | 行为标签 | ✅ *新增* |
...
### 6.1 莽夫型 (The Rusher) — Phase 1 首发原型 `✅ 已完成`
- **对应**：深渊爬行者
- **逻辑**：发现 → 全速直线冲锋 → 距离 < AttackRange → 播放咬合前摇 → 造成伤害 → 停顿恢复 ✅
- **表现**：像疯狗一样，依靠数量取胜，给玩家巨大压迫感 ✅
- **特征参数**：高 MoveSpeed、短 TelegraphDuration、短 AttackRange、长 RecoveryDuration ✅ SO 已配置
- **技术重点**：群聚算法（Boids Separation），多只同时出现时像虫群一样自然散开 🟡 待实现
- **实现**：`EnemyBrain` 基类 + `EnemyStatsSO` Rusher 预设，通过 `EnemyAssetCreator` 一键创建资产
- **Play Mode 验证**：完整伤害链路通畅，死亡回池+延迟重生正常 ✅

### 6.2 射手型 (The Shooter / Kiter) — Phase 1+ `✅ 已完成`
- **对应**：工蜂无人机（原"腐化漂浮者"）
- **逻辑**：
  - 距离 > PreferredRange：靠近 (Chase) ✅
  - 距离 < RetreatRange：逃离（反向移动）✅ `RetreatState`
  - 距离合适：停下 → 前摇（sprite 红闪）→ 三连发投射物 → 恢复硬直 ✅ `ShootState`
- **表现**：远程骚扰者，迫使玩家在处理近战怪的同时切后排 ✅
- **特征参数**：MoveSpeed=3.5、SightRange=16、PreferredRange=10、RetreatRange=5、ShotsPerBurst=3 ✅ SO 已配置
...
```
Phase 1: AI 基础框架                                     ✅ 全部完成
  ├─ HFSM 状态机框架（纯 C# 类）                          ✅ IState + StateMachine
  ├─ 感知系统（视觉 + 听觉 + 记忆衰减）                    ✅ EnemyPerception
  ├─ 基础状态集（Idle/Chase/Engage/Return）               ✅ 4 个外层状态
  ├─ 攻击子状态（Telegraph/Attack/Recovery）               ✅ 3 个内层子状态
  ├─ EnemyStatsSO 数据结构（近战+远程+感知+抗性+标签）       ✅ 30+ 字段
  ├─ IDamageable 接口 + 四种攻击源桥接                     ✅ Projectile/Laser/Echo/Melee
  ├─ 玩家受伤闭环（ShipHealth : IDamageable）              ✅ 击退+闪白+死亡
  ├─ 对象池生成器（EnemySpawner）                          ✅ Pool + Round-robin + 重生
  ├─ 第一个可玩原型：莽夫型 (The Rusher)                    ✅ Play Mode 验证通过
  ├─ 第二个可玩原型：射手型 (The Shooter)                   ✅ ShooterBrain + EnemyProjectile
  ├─ Editor 工具（EnemyAssetCreator + BestiaryImporter）   ✅ 一键资产创建 + CSV 导入
  └─ 怪物图鉴数据（Enemy_Bestiary.csv 26 种敌人）           ✅ P1+P2 完整数值

Phase 1 遗留项（已补完）:
  ├─ 韧性 (Poise) 运行时逻辑                              ✅ StaggerState + OnPoiseBroken 事件 + SuperArmor 豁免
...
- 感知系统视觉检测频率 ≤ 5Hz（每 0.2 秒），大量敌人时不卡 ✅ `_visionCheckInterval = 0.2f`
- 听觉事件采用广播 + 距离过滤，不用物理碰撞体 ✅ `StarChartController.OnWeaponFired` 静态事件
- 所有敌人实体使用对象池管理（复用已有 `PoolManager`）✅ `EnemySpawner` + `GameObjectPool`
- HFSM 为纯 C# 类，无 MonoBehaviour 开销 ✅ `StateMachine` + `IState` 均为纯 C#
- 敌人使用 "Enemy" Layer，碰撞矩阵按需配置 ✅ Physics2D 碰撞矩阵已设置
