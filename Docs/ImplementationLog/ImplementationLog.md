...
## SAIL Slot Count Inspector Field — 2026-03-07 10:25

### 修改文件
- `Assets/Scripts/UI/StarChartPanel.cs`

### 内容简述
在 `StarChartPanel` 的 `[Header("Shared SAIL Column")]` 区域新增 `[SerializeField] [Range(1, 4)] private int _sailSlotCount = 1;` 字段，并修改 `RefreshSharedSailColumn()` 使用该字段控制解锁格子数量（`i < _sailSlotCount`）。

### 目的
让设计师/开发者可以直接在 Inspector 中调整 SAIL 可用格子数（1~4），无需改代码。

### 技术方案
- 新增 `_sailSlotCount`（Range 1-4，默认 1）
- `RefreshSharedSailColumn()` 中将 `i == 0` 改为 `i < _sailSlotCount`
- `HasSpaceForItem` 委托保持原逻辑（Controller 目前只支持1个 SAIL 装备，格子数量仅影响 UI 显示）

---

## 槽位层统一化：SAT/Core/Prism/SAIL — 2026-03-07 11:00

### 新建/修改文件
- `Assets/Scripts/Combat/StarChart/WeaponTrack.cs` — 修改
- `Assets/Scripts/Combat/StarChart/LoadoutSlot.cs` — 修改
- `Assets/Scripts/Combat/StarChart/StarChartController.cs` — 修改
- `Assets/Scripts/Core/Save/SaveData.cs` — 修改
- `Assets/Scripts/UI/TrackView.cs` — 修改
- `Assets/Scripts/UI/StarChartPanel.cs` — 修改
- `Assets/Scripts/UI/SlotCellView.cs` — 修改
- `Assets/Scripts/UI/DragDrop/DragDropManager.cs` — 修改

### 内容简述
统一 SAT / SAIL 的槽位层架构，使其与 Core/Prism 保持一致。

### 目的
1. SAT 支持动态列扩展（SetLayerCols）和 Inspector 调控（_debugSatCols）
2. SAIL 改用 SlotLayer<LightSailSO> 架构，支持多格扩展和 Inspector 调控（_debugSailCols）
...
# = 
### 新建/修改文件
- `Assets/Scripts/Combat/StarChart/Wea?? - `Assets/Scripts/Combge- `Assets/Scripts/Combat/StarChart/LoadoutSlot.cs` — 修?s- `Assets/Scripts/Combat/StarChart/StarChartController.cs` ??- `Assets/Scripts/Core/Save/SaveData.cs` — 修改
- `Assets/ScriptSa- `Assets/Scripts/UI/TrackView.cs` — 修改
- `ALi- `Assets/Scripts/UI/StarChartPanel.cs` —  ? `Assets/Scripts/UI/SlotCellView.cs` — 修改
il- `Assets/Scripts/UI/DragDrop/DragDropManager.c??### 内容简述
统一 SAT / SAIL 的槽位层架tSail` 支???一 SAT / SAnc
### 目的
1. SAT 支持动态列扩展（SetLayerCols）和 Inspector 谰?. SAT 攓 2. SAIL 改用 SlotLayer<LightSailSO> 架构，支持多格扩展和 Inspector 调?-
### 技术方案
...
## 槽位层统一化?at
**SAT（WeaponLay- `_satLayer` 初始列数kS---
...
- bug- `tCo
# = 
### 新建??0=使?##?? `Assets/Scripts/CombeF- `Assets/ScriptSa- `Assets/Scripts/UI/TrackView.cs` — 修改
- `ALi- `Assets/Scripts/UI/StarChartPanel.cs` —  ? `Assets/Scripts/UI/SlotCellView.cs` — 修改
il- `Assets/Scripts/UI/DragDrop/DragDropManager.c??### 内容简述
统?e- `ALi- `Assets/Scripts/UI/StarChartPanel.cs` —  ? `Assets/??il- `Assets/Scripts/UI/DragDrop/DragDropManager.c??### 内容简述
统一 SAT / SAIL 的槽位层???一 SAT / SAIL 的槽位层架tSail` 支???一 SAT / SAnc
### ?T### 目的
1. SAT 支持动态列扩展（SetLayerCols）和Ex1. SAT ?i### 技术方案
...
## Bug Fix: FractalPrism_TwinSplit 旧版 Asset 数据修复 — 2026-03-07 11:39

### 修改文件
- `Assets/_Data/StarChart/Prisms/FractalPrism_TwinSplit.asset` — 修改

### 内容简述
修复 FractalPrism_TwinSplit.asset 中残留的旧版序列化数据，将 `_slotSize: 2` 改为 `_slotSize: 1`，并移除 `_shape: 1`（Shape1x2H）。

### 目的
Twin Split 在 Inspector 中显示为 Shape1x2H（占 2 格），导致无法放入 Prism 槽位（Prism 默认 1 列 × 2 行，Shape1x2H 需要 2 列）。

### 技术方案
直接编辑 .asset 序列化文件，与 AssetCreator 代码（`_slotSize: 1`，无 `_shape` 字段即默认 Shape1x1）保持一致。ShebaP_TwinSplit.asset 本身数据正确，无需修改。

---

## Bug Fix: Track 内部 Slot 自由移动 — 2026-03-07 11:50

### 修改文件
- `Assets/Scripts/UI/TrackView.cs` — 新增 `HasSpaceForItemExcluding` 方法
- `Assets/Scripts/UI/SlotCellView.cs` — 修改 `ComputeDropValidity` 中的 `hasSpace` 计算
- `Assets/Scripts/UI/DragDrop/DragDropManager.cs` — 修复 `ExecuteDrop` same-track no-op
- `Assets/Scripts/UI/StarChartPanel.cs` — 修复 SAIL `HasSpaceForItem` delegate

### 内容简述
修复已装备在 Track 上的部件（Core / Prism / SAT / SAIL）无法通过拖拽移动到同一 Track 其他 Slot 的 Bug。

### 根本原因（两处）
1. **`DragDropManager.ExecuteDrop()`**：`sourceTrack == DropTargetTrack` 时直接 no-op，不执行任何移动。
2. **`SlotCellView.ComputeDropValidity()`**：`hasSpace` 计算没有排除被拖动 item 自身，导致同 Track 内移动时 `hasSpace = false`，错误地显示橙色 Replace 高亮而非绿色 Valid。

### 技术方案

**TrackView.HasSpaceForItemExcluding（新增）**
- 临时 `Unequip` 被拖动 item，调用 `FindFirstAnchor` 检查剩余空间，再用 `TryPlace` 恢复到原始 anchor 位置。
- 使用 `TryPlace`（而非 `TryEquip`）确保 item 精确恢复到原始坐标，不破坏 layer 状态。

**SlotCellView.ComputeDropValidity**
- 新增 `isDraggingFromSameTrack` 判断（`payload.Source == DragSource.Slot && payload.SourceTrack == OwnerTrack.Track`）。
- Core/Prism：同 track 时调用 `HasSpaceForItemExcluding`，跨 track 时调用原有 `HasSpaceForItem`。
- SAT：同 track 时将 `FreeSpace + draggedSat.SlotSize > 0` 作为 `hasSpace`（排除自身 1 格）。
- SAIL：通过 `StarChartPanel` 注入的 delegate 处理，delegate 内检查 `SailLayer.Items.Contains(draggedSail)` 并排除自身。

**DragDropManager.ExecuteDrop**
- 移除 `sourceTrack == DropTargetTrack` 时的 no-op。
- 改为：用 `GetAnchor` 获取 item 当前 anchor，若与 `DropTargetAnchorCol/Row` 不同则执行 `UnequipFromTrack` + `EquipToTrack`；相同则真正 no-op（放回原位）。

**StarChartPanel SAIL delegate**
- 修复 SAIL `HasSpaceForItem` delegate：当被拖动的 SAIL 已在 layer 中时，计算 `FreeSpace + draggedSail.SlotSize > 0`（排除自身），而非直接 `FreeSpace > 0`。

---

## Bug Fix: SAIL 列内自由移动（重复图标修复）— 2026-03-07 21:35

### 修改文件
- `Assets/Scripts/UI/SlotCellView.cs` — 修改 `ComputeDropValidity` 中 SAIL 空间计算与 no-op 检测
- `Assets/Scripts/UI/DragDrop/DragDropManager.cs` — 修复 `ExecuteDrop` SAIL 列内移动逻辑 + `EquipToTrack` 防御性检查

### 内容简述
修复将已装备的 SAIL 从 SAIL 列的一个格子拖动到另一个格子时，出现两个重复 SAIL 图标的 Bug。

### 根本原因（三处）
1. **`DragDropManager.ExecuteDrop()` 的 `DropTargetIsSailColumn` 分支**：直接调用 `EquipToTrack(item, null)`，没有先判断 SAIL 是否已在列中，也没有先执行 `UnequipLightSail` 将其从原位置移除，导致同一 SAIL 同时存在于两个格子。
2. **`EquipToTrack()` 的 `LightSailSO` 分支**：没有防御性 Unequip 自身的逻辑，当 SAIL 已在 layer 中时直接 `EquipLightSail` 会产生重复。
3. **`SlotCellView.ComputeDropValidity()` 的 `HasSpaceForItem` 分支**：SAIL 列内拖动时没有排除自身占用的空间，导致空间判断不准确（与 Track 内移动 Bug 同类问题）。

### 技术方案

**DragDropManager.ExecuteDrop — SAIL 列内移动**
- 在 `DropTargetIsSailColumn` 分支中，检测是否为列内移动（`payload.Source == DragSource.Slot` 且 SAIL 已在 `SailLayer.Items` 中）。
- 若是列内移动：先用 `GetAnchor` 获取当前 anchor，若与目标 anchor 相同则 no-op；否则先 `UnequipLightSail` 再 `EquipToTrack`。
- 若来自 Inventory（非列内移动）：保持原有逻辑不变。

**DragDropManager.EquipToTrack — LightSailSO 防御性检查**
- 新增 `sailAlreadyEquipped` 检测：若 SAIL 已在 `SailLayer.Items` 中，先 `UnequipLightSail` 再 `EquipLightSail`，跳过驱逐逻辑（驱逐逻辑仅对来自 Inventory 的新装备生效）。
- 保留原有驱逐逻辑（`sailOccupant != null && !ReferenceEquals(sailOccupant, sail)` 以及 `FreeSpace <= 0` 时驱逐第一个）仅在非列内移动时执行。

**SlotCellView.ComputeDropValidity — SAIL 空间计算**
- `HasSpaceForItem != null` 分支中，检测是否为 SAIL 列内拖动（`payload.Source == DragSource.Slot` 且 SAIL 已在 `SailLayer.Items` 中）。
- 若是列内拖动：用 `sailLayer.FreeSpace + draggedSail.SlotSize > 0` 排除自身后判断空间。
- 在 SAIL anchor 计算之后，新增 no-op 检测：若目标 anchor 与当前 anchor 相同，直接返回 `Valid`（no-op 状态）。

---

## StarChart Asset 数据修复（旧版本字段补全）— 2026-03-08 00:49

### 修改文件
- `Assets/_Data/StarChart/Cores/ShebaCore_PulseWave.asset`
- `Assets/_Data/StarChart/Prisms/ShebaP_Bounce.asset`
- `Assets/_Data/StarChart/Prisms/ShebaP_TwinSplit.asset`
- `Assets/_Data/StarChart/Prisms/ShebaP_RapidFire.asset`
- `Assets/_Data/StarChart/Prisms/ShebaP_Boomerang.asset`
- `Assets/_Data/StarChart/Cores/ShebaCore_FocusLaser.asset`
- `Assets/_Data/StarChart/Cores/LightCore_BasicLaser.asset`
- `Assets/_Data/StarChart/Cores/AnomalyCore_Boomerang.asset`
- `Assets/_Data/StarChart/Cores/MatterCore_StandardBullet.asset`
- `Assets/_Data/StarChart/Sails/ShebaSail_Scout.asset`
- `Assets/_Data/StarChart/Sails/ShebaSail_Standard.asset`
- `Assets/_Data/StarChart/Sails/TestSpeedSail.asset`

### 内容简述
全量检查并修复所有 StarChart 部件 asset 文件中的旧版本数据问题：
1. **`ShebaCore_PulseWave`**：`_shape` 字段值错误（4=ShapeLMirror，3格），修正为 5（Shape2x2，4格），与 `_slotSize=4` 保持一致。
2. **11 个部件缺少 `_shape` 字段**：这些部件在 `_shape` 字段加入 SO 之前创建，Unity 序列化时未写入该字段，导致运行时读取默认值 0（Shape1x1）。显式补全 `_shape: 0` 确保数据与代码行为一致，消除隐式依赖。

### 修复后各部件形状一览

| 部件 | `_shape` | `_slotSize` | 形状 |
|------|---------|------------|------|
| ShebaCore_MachineGun | 1 | 2 | Shape1x2H |
| ShebaCore_Shotgun | 3 | 3 | ShapeL |
| ShebaCore_PulseWave | **5** | 4 | Shape2x2 ✅修复 |
| EchoCore_BasicWave | 2 | 2 | Shape2x1V |
| ShebaCore_FocusLaser | **0** | 1 | Shape1x1 ✅补全 |
| LightCore_BasicLaser | **0** | 1 | Shape1x1 ✅补全 |
| AnomalyCore_Boomerang | **0** | 1 | Shape1x1 ✅补全 |
| MatterCore_StandardBullet | **0** | 1 | Shape1x1 ✅补全 |
| ShebaP_Homing | 2 | 2 | Shape2x1V |
| ShebaP_MinePlacer | 3 | 3 | ShapeL |
| ShebaP_Bounce | **0** | 1 | Shape1x1 ✅补全 |
| ShebaP_TwinSplit | **0** | 1 | Shape1x1 ✅补全 |
| ShebaP_RapidFire | **0** | 1 | Shape1x1 ✅补全 |
| ShebaP_Boomerang | **0** | 1 | Shape1x1 ✅补全 |
| ShebaSail_Scout | **0** | 1 | Shape1x1 ✅补全 |
| ShebaSail_Standard | **0** | 1 | Shape1x1 ✅补全 |
| TestSpeedSail | **0** | 1 | Shape1x1 ✅补全 |

### 目的
消除旧版本 asset 数据中 `_shape` 字段缺失导致的隐式依赖，确保所有部件的序列化数据与 `ItemShape` 枚举定义完全对齐，防止未来新增形状时出现意外的默认值行为。
