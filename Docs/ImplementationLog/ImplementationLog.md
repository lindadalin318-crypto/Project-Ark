...
### DemonHR V4.0 — 注意力分层架构整合 (2026-03-18 23:50)

**新建/修改文件：**

- `SideProjects/DemonHR/MVP_Design_V4.md`（新建，从 V3.1 复制后大幅修改）
- `Docs/ImplementationLog/ImplementationLog.md`

**内容：**

1. 版本号从 V3.1 升级到 V4.0（"注意力手术刀版"），更新版本变更摘要，新增 V4.0 变更表（3 条核心改动）和更新后的系统模块清单。
2. 在第四章（单季度核心循环）新增 **4.1 注意力分层架构 (Attention Triage)** 小节，将所有决策轴分为三层：
   - **前台决策**（每回合 3 个）：分配情报带宽、打出卡牌、选择破坏/保护方向
   - **告警决策**（后台运行，仅阈值弹出）：暴露/可疑度、魔王忠诚度、MICE 培养进度、资产维护、团队凝聚力
   - **被动系统**（完全自动）：信任矩阵、压力值、人格激活、连锁反应、优雅度评价
   - 含认知负荷审计表，证明最坏情况仍在人类工作记忆极限内
3. 重构 **4.2 单回合四阶段**，为每个阶段标注注意力层级归属（前台/被动），并在阶段二和阶段四中说明被动系统的更新时机。
4. 第四章所有后续小节重新编号（4.2→4.3 主动情报系统, 4.3→4.4 暴露风险系统, 4.4→4.5 左侧群聊, 4.5→4.6 操作链, 4.6→4.7 Reaction Window, 4.7→4.8 胜利条件）。
5. 重构 **第六章渗透者系统**：从"每回合 4 种手动指令"改为 **策略设定模式**——入场时一次性配置社交倾向/目标锁定/情报策略/激进度，之后自动执行，仅在高价值情报、关键破坏机会、暴露临界、目标求助四种情况弹出确认。认知负荷从每回合 4 次操作降至每 2-3 回合 1 次确认。
6. 重构 **8.3 汇报节点**：从模糊的"回合内额外行动"改为 **地图事件节点**——在季度小地图上作为固定遭遇出现，有专属 UI 场景（魔王通讯频道），玩家可提前在地图上看到并规划。
7. 修复第九章（角色持续化）的小节编号错误（8.1→9.1, 8.2→9.2, 8.3→9.3）。
8. 更新 **14.3 MVP 收束建议**：新增注意力分层和渗透者策略设定两个 MVP 条目。
9. 更新 **14.4 技术风险评估**：新增注意力分层 UI 设计风险和渗透者策略设定缺乏操控感的风险。
10. 更新 **第十五章设计结论**：反映 V4.0 的核心改动（每回合 3 决策、策略设定模式、地图汇报节点），新增设计哲学第二条（"复杂度不等于深度"），重新排列下一步优先级（注意力分层 UI 原型排第一）。

**目的：**

- 解决 V3.1 评审暴露的首要问题——系统过载（10+ 并行决策轴同时争夺玩家注意力），通过注意力分层架构将每回合的主动决策压缩至 3 个，同时保留所有系统的深度。
- 核心理念：不砍系统，而是按"需要玩家干预的频率"分层，让游戏的丰富度在后台运转而非在前台堆叠。

**技术：** 注意力分层（Foreground/Alert/Passive 三层架构）、策略设定模式（渗透者从手动操作改为策略+自动执行+关键弹出）、地图事件节点（汇报节点从回合行动改为独立遭遇）、认知负荷审计、告警队列优先级排序。

---

### DemonHR V4.2 — 雾灯情报模型 + 空包弹防御 + 角色情报拼图整合（2026-03-18 23:55）

**修改文件：**

- `SideProjects/DemonHR/MVP_Design_V4.md`（大量修改）

**内容：**

三个紧密关联的情报系统设计方案整合进 V4，版本号从 V4.1 升级至 V4.2：

1. **雾灯模型 (Fog Lamp)**：重写 4.3 主动情报系统。所有频道永远可见（免费摘要+热度信号 🔴🟡🟢🔵），AP 解锁的是"精度"而非"访问权"。未付费的私聊不再显示"加密通讯—内容未知"，而是显示有意义的一句话摘要+热度指示器。

2. **"空包弹"防御**：在 4.3 中引入三种情报货币（战术情报/战略情报/档案情报），设计铁律"任何花了 AP 的频道必定返回至少一条有价值信息"。UI 重新框定"排除型"收获为正面发现。

3. **角色情报拼图 (Intel Jigsaw)**：新增 9.4 子章节。每个角色是一张 12-15 碎片的拼图面板，含四类碎片（公开/情报/时机/深层）。时机碎片只在特定游戏状态窗口出现。关键战略碎片（MICE 可行/锁死）有 2-3 条冗余发现路径防止"错过即死路"。拼图碎片直接影响游戏机制（创伤倍率、MICE 路线解锁等）。

**同步修改：**
- 4.1 注意力分层前台决策 #1：更新为"选择情报投资"（基于雾灯免费摘要后决策）
- 5.3 资源经济："看 vs 做"示例更新为雾灯模型下的决策场景
- 5.3 跨季度资源：人事档案描述更新为"情报拼图碎片"载体
- 10.3 情报成长：新增情报拼图持续化引用
- Demo 推演（第十四章）：三个回合全部重写——加入免费层摘要展示、热度信号引导、情报拼图更新反馈
- Demo 验证结论：扩充至 10 条，新增雾灯/三种货币/拼图进度感的验证点
- 版本头+变更日志：新增 V4.2 变更块，描述三个新问题及解决方案
- 系统模块清单：新增"主动情报系统—雾灯模型"和"角色情报拼图"两行
- MVP 收束建议（15.3）：新增雾灯模型和情报拼图两行
- 技术风险（15.4）：新增"热度信号无风险"、"碎片总量爆炸"、"时机窗口过窄"三条
- 设计结论（16）：新增雾灯/情报货币/拼图三条核心方向描述
- 下一步优先级：新增 #12 热度信号校准、#13 情报蓝图首批规划、#14 情报拼图 UI 原型

**目的：**

- 解决"花 AP 可能什么都看不到"的空包弹问题——所有投资必有回报
- 解决"精心设计的角色对话可能永远不被发现"的内容浪费问题——免费层确保摘要永远可见
- 将零散的角色信息整合为可视化拼图系统，创造收集驱动力和跨季度情报资产积累
- 让 MICE 策反从"一开始就看到四维雷达"变为"通过情报拼图逐步确认哪条路可行"

**技术：** 雾灯双层信息架构（free summary + paid detail）、热度信号系统（🔴🟡🟢🔵 四级）、三种情报货币（tactical / strategic / dossier）、情报拼图四类碎片模型（public / intel / contextual / deep）、冗余发现路径设计（关键碎片 ≥ 2-3 路径）、情报蓝图工具（角色设计前置规划）。

---

### DemonHR V4.0 — 文本生产管线（五层防线架构）整合 (2026-03-19 00:30)

**修改文件：**

- `SideProjects/DemonHR/MVP_Design_V4.md`
- `Docs/ImplementationLog/ImplementationLog.md`

**内容：**

1. 新增 **第十二章：文本生产管线 — 五层防线架构 (Text Production Pipeline)**，完整的文本生产方案：
   - **12.1 问题量化**：估算原始文本需求（~2,400 条消息），明确问题规模
   - **12.2 第一层：设计层减量**：短消息格式（每条 5-15 字）、非文字信息载体（已读/未读、输入中气泡、表情反应、语音消息占位、在线状态等可传递 30-40% 信息量）、信息密度三级分级（A 级精写 20% / B 级模板 50% / C 级固定 30%），将精写量从 2,400 条降至 ~480 条
   - **12.3 第二层：消息 DNA 系统**：按"意图×情绪×人格×上下文"构建消息家族，80 个家族 × 6 人格变体 = 480 条短消息，总文本量 5,000-7,000 字，1-2 周工作量
   - **12.4 第三层：对话编排引擎**：35 个对话序列模板（定义起承转合节奏）、上下文感知插值（人格/情绪/关系/具体指代四层选择逻辑）、非文字元素运行时编排（压力→回复延迟、低信任→不显示已读等）
   - **12.5 第四层：关键场景手写**：75 段手写文本（MICE 策反对话 32 段 + 崩溃独白 16 段 + 汇报节点 12 段 + 结局 5 段 + Q1 教学 10 段），覆盖 80% 的玩家情感记忆
   - **12.6 第五层：验证管线**：Text Taste Test（盲测 5 人"想不想继续读"）、重复度衰减审计、角色声音一致性检查、跳过率监测
   - **12.7 工作量总览**：总计 6-8 周（单人），五层各自独立可拆分
2. 后续章节重新编号：原十二→十三（多结局）、原十三→十四（Demo）、原十四→十五（待讨论）、原十五→十六（设计结论），含所有内部小节编号更新（12.x→13.x）
3. 替换 **15.2 文本生成方案**：从旧的"三种方案对比"替换为五层防线架构摘要 + 指向第十二章的引用
4. 更新 **版本变更摘要**：V4.0 变更表新增"文本质量致命依赖"行，系统模块清单新增"文本生产管线"
5. 更新 **15.3 MVP 收束**：新增"文本管线"MVP 条目（消息 DNA 前 40 家族 + 15 个对话模板 + Q1 手写）
6. 更新 **15.4 技术风险**：文本质量风险的缓解策略更新为五层防线引用
7. 更新 **第十六章设计结论**：新增文本管线相关方向描述、新增第三条设计哲学（"地基先于楼层"）、Text Taste Test 提升为下一步优先级第一项（生死门）、新增"消息 DNA 首批写作"验证步骤

**目的：**

- 解决 V3.1 评审暴露的致命风险——"文本质量是致命依赖且无解"。通过五层防线架构将不可能的文本量拆解为可估算、可拆分、可验证的生产任务。
- 核心理念：不是"写更多文字"，而是"用更少的文字传递更多信息"——短消息、非文字元素、消息家族系统共同将写作量降低 80%，同时保持甚至提升"真实群聊"的感觉。

**技术：** 消息 DNA 系统（Intent×Emotion×Personality×Context 四维构建）、对话编排引擎（序列模板 + 上下文插值）、非文字元素编排（压力/信任/状态驱动的 UI 行为）、Text Taste Test 验证方法论、信息密度三级分级。

---

### DemonHR — 战斗模块替代方案备案库标准化 (2026-03-19 01:30)

**新建/删除文件：**

- 新建 `SideProjects/DemonHR/MVP_Combat_Alternatives.md`（从 MVP_Combat_Backup.md 重构）
- 删除 `SideProjects/DemonHR/MVP_Combat_Backup.md`（旧文件）
- `Docs/ImplementationLog/ImplementationLog.md`

**内容：**

将原始的 `MVP_Combat_Backup.md`（无标题、编号冲突、格式不统一的 8 个备案）重构为标准化的 `MVP_Combat_Alternatives.md`：

1. 新增文档标题和用途说明
2. 按设计范式分为四类：信息操纵（A1/A2）、角色扮演（B1/B2）、系统策略（C1/C2/C3）、议程控制（D1）
3. 统一每个方案的格式：核心概念 → 机制 → 优势 → 风险
4. 消除命名冲突（两个"方案 A"、两个"方案一"重新编号为 A1/A2/B1/B2/C1/C2/C3/D1）
5. 新增综合对比表（7 维度 ⭐ 评分）、推荐优先级排序、混搭建议

**目的：**

- 让战斗模块的所有替代方案有统一的查阅入口，便于后续比较和决策
- 消除命名混乱，建立清晰的分类体系

**技术：** 文档标准化、设计方案分类学（信息操纵/角色扮演/系统策略/议程控制四范式）、多维度评分矩阵。

---

### DemonHR V4.1 — 三层资源经济体系整合 (2026-03-19 04:00)

**修改文件：**

- `SideProjects/DemonHR/MVP_Design_V4.md`（V4.0 → V4.1）
- `Docs/ImplementationLog/ImplementationLog.md`

**内容：**

将 V4.0 独立的"预算"和"情报带宽"两套资源系统重构为统一的三层资源经济架构，全面更新文档中所有相关引用：

1. **版本升级**：V4.0 → V4.1，新增变更日志条目（四个问题→四个解决方案表格）
2. **5.3 资源经济体系**（核心重写）：
   - 第一层（回合级）：行动力 3 AP/回合（监听+出牌共池），手牌抽 3 张/上限 7
   - 第二层（季度级）：经费储备（Q1:8→Q4:15，可提取到 AP）、暗线网络（低暴露积累→零暴露操作）、权限等级
   - 第三层（跨季度）：人事档案（情报折扣）、魔王信任（→下季度储备）、公会档案（→下季度敌方反制）
   - 资源流动全景图（ASCII 流程图）
   - 卡牌费用与行动力关系说明
3. **4.1 注意力分层**：前台决策 #1 从"分配情报带宽"改为"分配行动力到情报频道"
4. **4.2 回合阶段**：侦察阶段引入经费储备提取，干预阶段改为"消耗剩余行动力"
5. **4.3 主动情报系统**：全面重写，带宽点数→行动力消耗，策略价值改为"看vs做"四种分配模式
6. **10.4 资源成长体系**：从"情报带宽成长"扩展为行动力成长+经费储备成长+情报成本降低三维
7. **散落引用更新**（约 15 处）：卡牌双面性/MICE 资产维护/魔王忠诚度奖励/汇报节点费用等处的"预算→行动力"、"带宽→行动力"
8. **Demo 推演**（三回合）：更新为 V4.1 资源标记，加入经费储备提取和暗线积累演示
9. **MVP 收束建议**：新增"资源经济"行
10. **技术风险评估**：新增三条风险（行动力统一池开局窘迫/经费储备节奏直觉性/暗线网络速率调参）
11. **设计结论**：新增资源经济三条方向、新增"资源是叙事"设计哲学、新增三条下一步验证项

**目的：**

- 解决 V4.0 资源系统"各自独立、缺乏经营感"的问题
- 通过合并预算和带宽为统一行动力，制造"看什么 vs 做什么"的永恒核心取舍
- 引入经费储备和暗线网络，增加季度级中期经营维度
- 通过跨季度资源联动（信任→储备、档案→情报折扣），让战役级选择有实际的机制后果

**技术：** 统一资源池设计（Action Points 合并 Budget + Bandwidth）、季度总量资源（Reserve Fund 提取机制）、政治资本系统（Shadow Network = 优雅度的资源化奖励）、三层资源联动（回合→季度→战役）。
...
**技术：** `SharedDecisionContext` 需求信号复用、`RelicData` 标签语义扩展、STS2 反编译效果对表、低改动面的评分链细化策略。
