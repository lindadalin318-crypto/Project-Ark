...
## StarChart Refactor Plan 收官：下游文档协同 + Plan 归档 — 2026-04-26 09:20

### 修改文件

- `Docs/2_TechnicalDesign/Combat/StarChart_CanonicalSpec.md`（§1.1 Runtime 清单 + §2 Satellite 循环链 owner + §3.5/§3.6/§3.7 Spawn pipeline owner + §4.1 Save/Load owner + §5.4 Modifier 深拷贝 owner + §10.1 "无 fallback 旁路"扩写为四类协作器）
- `Docs/2_TechnicalDesign/Combat/StarChart_AssetRegistry.md`（§0 字段命名口径更新为"`_slotSize` 已于 2026-04-25 代码层移除" + §3.2 Legacy 段落改写 + §4 Prism 表格 DisplayName + `ShebaP_Homing._family` 状态修正 + §4.1 DisplayName 冲突治理小节重写 + §4.2 Tint 通道生效清单更新为 2/9 + 顺手修正 `PrismFamily` 枚举值笔误 `3 = Tint` → `2 = Tint` + §4.3 磁盘字段缺失段落标注 ✅ 已修复）
- `Docs/6_Diagnostics/StarChart_Architecture_Audit.md`（§7.1 P1 全部 ✅ 附批次索引 + §7.2 P2 全部 ✅ 附批次索引 + §7.3 P3 第 10 项 ✅ 含磁盘 YAML 孤儿字段清除策略 + §7.3 其余 ⏸ 未触发 + 尾部改造落地方案链接从 ongoing 改为 complete）
- `Docs/6_Diagnostics/StarChart_Components_Inventory.md`（§0 TL;DR 按 Creator 权威真相源重新分类 Prism 完成度 + §3 主表格 `_family` 真相列改写 + §3.1 Action Items 逐项标注 ✅ 已完成 / ❌ 撤销 / ⏸ 待决策 + §6 红绿灯总览 🔴 Broken 清零，🟢 Functional 11→13，🟡 Partial 分布按 Creator 真相更新 + §7 系统性观察 "最大数据债"改写为 owner 真相复盘 + §7.2 Modifier 共用关系补注入状态列 + §7.3 Prism Tint 注入 1/9→2/9 + §8 下一步按治理后 ROI 重排）
- `Implement_rules.md` §13（§13.3.2 硬约束 `StarChartController.SpawnProjectile` → `ProjectileSpawner.SpawnProjectile` + §13.4.6 新 Core 忘记在 switch 加分支的防御描述 owner 更新 + §13.5 验收清单第 8 条 owner 更新）
- `Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md` → `Docs/0_Plan/complete/2026-04-25-starchart-refactor-plan.md`（Plan 从 ongoing 归档到 complete，顶部"状态"行从"Plan 草稿，等待 owner 审阅后分批执行"改为"✅ 已完成并归档（2026-04-26）"，显式列出四个批次全部落地 + 协同表全部勾选 + 指向 ImplementationLog 的追溯链接）
- `Docs/5_ImplementationLog/ImplementationLog.md`（追加本条）

### 内容

执行 `Docs/0_Plan/complete/2026-04-25-starchart-refactor-plan.md` §十一"与上下游文档的协同"表列出的五份下游文档收尾对齐 + Plan 本身的归档动作，彻底闭合 StarChart 治理这一整轮改造。

**CanonicalSpec 协同**：Plan §十一 要求"`StarChartController` 拆分后同步各类 owner"。按拆分后真实 owner 对代码 + `.asset` 逐一校对更新：§1.1 Runtime 代码清单把 `LoadoutManager.cs` / `ProjectileSpawner.cs` / `StarChartSaveSerializer.cs` 三个新协作器及其职责补齐；§2 Satellite 循环链从 `StarChartController.Update → SatelliteRunner.Tick` 更新为 `StarChartController.Update → LoadoutManager.TickActive(dt) → SatelliteRunner.Tick(dt)`；§3.5 Spawn owner 从 `StarChartController.SpawnProjectile` 改为 `ProjectileSpawner.SpawnProjectile`（internal，Controller.ExecuteFire 调用），§3.6 InstantiateModifiers owner 改为 `ProjectileSpawner.InstantiateModifiers`（private static），§3.7 "每个 Core 每发"改为 `ProjectileSpawner.SpawnProjectile`；§4.1 `ImportFromSaveData` 标注 Controller 保留对外 API、内部委托 `StarChartSaveSerializer.Import`；§5.4 Modifier 深拷贝 owner 从 Controller 改为 ProjectileSpawner；§10.1 "无 fallback 旁路"声明从单一 Controller 扩展为"Controller + LoadoutManager + ProjectileSpawner + StarChartSaveSerializer 四者组成的现役主链"。

**AssetRegistry 协同**：L3-2 `_slotSize` 字段移除 + L1-4 `ShebaP_Homing._family` 补字段 + L2-3 DisplayName 去重 三项硬变更同步。§0 字段命名口径："_slotSize" 从"仅保留字段"改为"**已于 2026-04-25 从 `StarChartItemSO.cs` 代码层移除**，YAML 孤儿字段 Unity 下次保存时自动清除"；§3.2 Legacy 段落 + 审计命令说明同步改写；§4 PrismSO 表格：`ShebaP_TwinSplit` DisplayName → `"Sheba Twin Split"`、`ShebaP_Boomerang` DisplayName → `"Sheba Boomerang"`、`ShebaP_Homing._family` → `Tint ✅ (L1-4 已补字段)`；§4.1 重写为"DisplayName 冲突治理（历史重名已全部消除）"，记录 L2-3 validator 首次运行发现 + 改名策略 + 存档迁移代价；§4.2 Tint 通道生效清单 Homing 从 ❌ 改为 ✅，其余 Sheba Prism 标注"按 Creator 权威保持"；更新过程中顺手发现并修复原文"`3 = Tint`"笔误，改为实际枚举值 `2 = Tint`；§4.3 磁盘字段缺失段落标题加删除线 + 「✅ 已修复（2026-04-25 L1-4）」标注，保留历史事实段落供追溯。

**Audit 协同**：§7.1 P1 四项全部标注 ✅ 完成 + 批次索引（批次 1 L1-1/L1-2/L1-3/L1-4），其中 L1-4 附加说明 "Boomerang / Bounce / MinePlacer 按 `ShebaAssetCreator` 代码真相保持不变，不再视为'错误'"——这是治理复盘中的关键认知修正：原诊断基于"命名暗示"推测 3 个 Prism 应当改为 Tint，但代码 owner 权威真相是保持 Rheology/Rheology/Fractal 不动。§7.2 P2 五项全部 ✅（L3-1 Controller 拆分 + L2-1/L2-2/L2-3/L2-4）；§7.3 P3 第 10 项 `_slotSize` 字段移除 ✅（批次 4 L3-2）+ 其余三项 ⏸ 未触发注明"需求驱动"。尾部改造落地方案链接从 `ongoing` 改为 `complete`，补充"下一步建议"的收官表达。

**Components Inventory 协同**：这是本次收尾改动最大的一份文档，因为它的 2026-04-25 初版本基于"命名和 Modifier prefab 暗示"做了大量推测性判断，治理执行后这些判断需要系统性纠正。§0 TL;DR Fully Functional 数量从 11 改为 13，Behavior-Broken 数量从 3 改为 0（"✅ L1-4 已修复（`ShebaP_Homing._family=2` 已补齐）"），新增"原始治理重心已落地 + 剩余设计层取舍（非 bug）"的复盘段落明确说明 `ShebaAssetCreator.cs` 是 Sheba 系列 Prism 的 owner 权威源，手改 asset YAML 属于低层修复手段不改变 owner。§3 主表格 "期望家族"列改为"按 Creator 权威是否意图"列，并对每个 Prism 的实际生效路径按治理后真相重写——例如 `ShebaP_Boomerang` 从 🔴 Broken 改为 🟡 Partial，标注"数值家族生效，Modifier 按设计舍弃"；`ShebaP_Homing` 从 🔴 Broken 改为 🟢 Functional，标注"✅ L1-4 补字段后 Tint 注入管线生效"。§3.1 Action Items 逐项标注：原 4 个 P0 中 `ShebaP_Homing` ✅ 已完成，其余 3 个 ❌ 撤销（按 Creator 真相不应修改）；新增 P1 建议"若未来决定让它们真正走 Tint 注入通道，应改 `ShebaAssetCreator.cs` 并重新生成资产，而不是手改 asset"；L2-3 DisplayName 唯一性 validator 状态备注已落地。§6 红绿灯总览重画：🔴 Broken 数量从 3 清零并显式写"治理后清零"，🟢 Functional 11 项改为 13 项（新增 `ShebaP_Homing`），🟡 Partial 6 项明细按"数值生效/Modifier 按 Creator 设计舍弃"重新描述。§7.1 "Prism `_family` 字段是当前最大的数据债"改写为 owner 真相复盘："真正的 bug 只有 `ShebaP_Homing` 一个，其余三个是合法的数值家族 Prism"；§7.2 Modifier 共用关系表新增"实际注入状态（治理后）"列，显式列出每个 Modifier 是否真的被注入；§7.3 Prism Tint 通道激活数从 "1/9" 提升为 "2/9"。§8 下一步按治理后 ROI 重排，P0 全部标记为已完成或已撤销，新建 P3"若设计同学决定让 Boomerang/Bounce/MinePlacer 真正注入，改 Creator 重新生成"。

**Implement_rules §13 协同**：§13.3.2 硬约束第 2 条 "`StarChartController.SpawnProjectile` 的 switch 是分发唯一权威" 改为 "`ProjectileSpawner.SpawnProjectile` 的 switch 是分发唯一权威" + 补充"`StarChartController.ExecuteFire` 会委托到 `ProjectileSpawner`"的链接说明；§13.4.6 "新 Core 忘记在 switch 加分支"陷阱的根因描述同步更新 owner；§13.5 验收清单第 8 条 `SpawnProjectile` owner 对齐。全项目 grep `StarChartController\.(SpawnProjectile|InstantiateModifiers|ImportFromSaveData|...)` 确认无其他过时引用残留。

**Plan 归档**：`mv Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md Docs/0_Plan/complete/`，对齐项目实际归档目录命名（`complete/` 而非 `done/`）。归档后 Plan 顶部"状态"行从"Plan 草稿，等待 owner 审阅后分批执行。本文档沉淀治理方案，不执行改动"改为"✅ 已完成并归档（2026-04-26）。批次 1 (L1-1 ~ L1-4) / 批次 2 (L2-1 ~ L2-4) / 批次 3 (L3-1 Controller 拆分) / 批次 4 (L3-2 `_slotSize` 字段移除) 全部落地，下游文档协同表（§十一）全部勾选"。

### 目的

**闭合 `2026-04-25-starchart-refactor-plan.md` 的最后两项（§十一 下游文档协同 + 归档）——这是整个 StarChart 治理改造的最后一步**。在 Plan §十"什么算全部完成"的硬验收标准下，本次之前已满足"四批次代码改动落地 + 代码层 `_slotSize` 消失 + validator 首次运行且修复重名"等客观技术条件，但"下游文档与代码 owner 同步"这一软条件始终存在最后一公里风险——**如果这五份文档不同步更新，未来任何人读到这些文档再动 StarChart 代码时都会被误导**（CanonicalSpec 里的旧 owner 指向被拆的 Controller 方法、AssetRegistry 里的 `_slotSize` 声明仍存、Components Inventory 里的 🔴 Broken 状态会让人去"修"其实按设计就是那样的 Prism、Implement_rules §13 的硬约束指向已不存在的方法）。因此本条 log 的关键价值**不是新增一项代码改动，而是让改造的认知面一致化**——代码 / 配置 / 规范文档 / 诊断文档 / 治理规则 五层全部收敛到同一个真相描述。

本次收尾中最值得 single-out 的决策是 **Components Inventory 的认知纠正**：原文 2026-04-25 把 `ShebaP_Boomerang` / `ShebaP_Bounce` / `ShebaP_MinePlacer` 标为 🔴 Broken，推测它们应当是 Tint——这个推测在治理执行过程中被证伪，因为 `ShebaAssetCreator.cs` 作为这些资产的 owner 权威源明确把它们设为 Rheology / Rheology / Fractal。**这是一次"owner 真相源原则"优先于"文档诊断推测"的教学样本**：当文档和代码冲突时，代码 is the source of truth；当修改资产字段时，应该先问"这个字段的 owner 是谁"，而不是"看起来应该是什么"。这个教训被固化进 Implement_rules 的未来扩展里（Components Inventory §3.1 新建议"若未来决定让它们真正注入 Modifier，改 Creator 重新生成"），避免未来再有人基于同样的"命名暗示"去改这些 asset 的 `_family`，引入新的 bug。

Plan 归档则完成项目"Plan 生命周期"的最后一环——`Docs/0_Plan/` 下 `ongoing` / `complete` / `specs` 三个目录的职责划分清晰：未完成 Plan 保留在 `ongoing`，完成后移入 `complete`，本次 StarChart Refactor Plan 从 2026-04-25 立档到 2026-04-26 归档，历时约 1 天完成 L1/L2/L3 三个层级共 11 个小任务 + 5 份下游文档协同更新，是项目近期最体系化的一次架构治理。

### 技术

**"下游文档协同"是架构治理的强制收尾阶段，不是可选的美化步骤**——如果把它跳过，改造实质上没完成，未来读文档的人会踩 owner 不一致的陷阱，相当于把当前的便利债务转嫁给下一次迭代。本次按 Plan §十一 协同表逐项对齐五份文档的执行过程中，三个最重要的技术要点值得长期记录：

**要点 1：Owner 真相源层级 = 代码 / .asset / .prefab > AssetRegistry > CanonicalSpec > Implement_rules**。当下游文档之间出现矛盾时，应该**沿这个层级回溯真相**而不是"哪份文档更新的就信哪份"。例如 Components Inventory 对 Prism `_family` 的判断冲突时，应该读 `ShebaAssetCreator.cs` + 读 `.asset` YAML 字段，而不是信任 Components Inventory 自己的"诊断结论"。这种回溯纪律在收尾阶段尤其重要，因为正是下游文档可能包含治理初期的推测性表述。

**要点 2：顺手修复 bug 要显式记录**。在更新 AssetRegistry §4.2 时发现了原文 "`3 = Tint`" 是笔误（实际 `PrismFamily` 枚举是 Fractal=0 / Rheology=1 / Tint=2），顺手在同一次编辑中修正。这类"发现并修复"必须在 log 里显式写出，而不是悄悄改掉——因为未来某个人做 git blame 看到这行被改过，如果没有 log 记录就无从得知是有意修正还是"可能引入错误的笔误修复"。显式记录是防止"悄悄的负向变更"的唯一手段。

**要点 3：文档归档是物理动作，不是口头声明**。归档 Plan 必须执行 `mv ongoing/X.md complete/X.md`（实际 `mv` shell 命令）+ 修改 Plan 顶部的"状态"行 + 更新下游文档里引用这份 Plan 的路径（Audit §7 尾部"改造落地方案链接"）。这三步缺一不可——如果只改 Plan 顶部的"状态"行但文件还留在 ongoing/，未来查 `ongoing/` 目录的人仍会认为它是未完成的 active Plan；如果只 mv 文件但不改状态行，打开 Plan 看到"Plan 草稿，等待 owner 审阅"还是会被误导；如果不同步 Audit 的路径链接，跨文档跳转就是死链。按 Implement_rules §3.9 "统一官方工具链"的精神，**归档路径必须是单一权威，三个位置必须同步**。

**MCP 使用补充**：本轮大量使用 `search_content` + `replace_in_file` 的组合精确修改 5 份文档的 20+ 处段落，其中 `Components Inventory` 的 §0 / §3 / §3.1 / §6 / §7 / §8 六个段落都是大段重写。`replace_in_file` 的"old_str 必须唯一"特性在这种场景下价值很大——通过足够的上下文锚点（如段落标题 + 首行）保证每次替换都精确命中，避免误改相邻段落。相比之前的"整文件 `write_to_file` 重建"风格，这种"段落级精确替换"更容易做精细化审查，也更不容易意外破坏未变更区域。

**下一步衔接**：StarChart 治理改造已全部完成，无"下一步"承接。Plan 文件已从 `ongoing` 归档到 `complete`，未来若需要做 StarChart 相关改动，应基于 `StarChart_CanonicalSpec.md` + `StarChart_AssetRegistry.md` + `Implement_rules §13` 三份当前最权威的文档工作，Plan 本身仅作为历史治理过程的追溯参考。若新增 CoreFamily / 新 Prism 类型等需求，按 Implement_rules §13.5 验收清单 + §13.6 推荐工作流执行即可，不需要复用本 Plan。

---

## StarChart L1-4：Sheba Homing Prism `_family` 字段漂移修复 — 2026-04-25 21:19

### 修改文件

- `Assets/_Data/StarChart/Prisms/ShebaP_Homing.asset`
- `Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md`

### 内容

以 `ShebaAssetCreator.cs`（L1 阶段已收口的 authored-only 工具）为权威真相源，审计全部 Prism `.asset` 的 `_family` 字段：

审计对象（9 个 Prism）与结论：

| # | asset | `.asset` 值 | Creator 权威 | 结果 |
|---|---|---|---|---|
| 1 | `ShebaP_TwinSplit` | `0` | Fractal(0) | ✅ |
| 2 | `ShebaP_RapidFire` | `1` | Rheology(1) | ✅ |
| 3 | `ShebaP_Bounce` | `1` | Rheology(1) | ✅ |
| 4 | `ShebaP_Boomerang` | `1` | Rheology(1) | ✅ |
| 5 | `ShebaP_MinePlacer` | `0` | Fractal(0) | ✅ |
| 6 | `ShebaP_Homing` | 缺字段→`0` | **Tint(2)** | 🔧 修复 |
| 7 | `FractalPrism_TwinSplit` | `0` | Fractal(0) | ✅ |
| 8 | `RheologyPrism_Accelerate` | `1` | Rheology(1) | ✅ |
| 9 | `TintPrism_FrostSlow` | `2` | Tint(2) | ✅ |

实际改动只有 1 处：`ShebaP_Homing.asset` 原本完全没有 `_family` 字段（Unity 反序列化为默认值 `0` = Fractal），但 `ShebaAssetCreator.CreateHomingPrism` 在代码里已声明为 `PrismFamily.Tint`。补字段位置参照 `ShebaP_Boomerang.asset` 的字段顺序（`_heatCost` 与 `_statModifiers` 之间）：
...
同步更新 `starchart-refactor-plan.md`：
1. L1-4 章节内容从"推测 5 处错误"改写为"实测 1 项漂移"的实际审计结果
2. 批次 1 清单尾行、批次 2 清单 L1-4 行、风险表行 3 全部更新为 ✅ 完成状态

### 前置审计（为何只有 1 项漂移）

Plan 原始版本基于 `Docs/6_Diagnostics/StarChart_Components_Inventory.md` §7 推测有 5 处错误（Homing 缺字段 + Boomerang/Bounce 的 Rheology 归属存疑 + MinePlacer 的 Fractal 归属存疑 + 第 5 项待明确）。实际对照 `ShebaAssetCreator.cs` 的权威 `PrismFamily` 声明后发现：

- Boomerang/Bounce/MinePlacer 的当前 `.asset` 值均与 Creator 一致，无漂移
- 真正漂移的只有 Homing（缺字段）
- 所以 Inventory §7 的"5 处"是基于设计质疑而非权威对照，经 Creator 审计后收敛为 1 处

### 目的

- 完成批次 2（L2 数据源收口）的最后一项遗留任务 L1-4，批次 2 整体 closeout
- 建立"`ShebaAssetCreator.cs` 是 Sheba Prism `_family` 权威源"的引用链，未来再跑一次 Creator 生成 asset 不会再产生漂移
- 修正 Plan 原文基于推测得出的过度估计（5 处），让后续路线图基于实测数据

### 技术

- YAML 字段插入（对照同类 asset 的字段顺序保持风格一致）
- Creator 作为权威源（Authored-only tool as SSoT）：遵循 `Implement_rules.md` §3.1 单一真相源原则
- 审计方法：对 `_family:` 做 workspace grep 找 `.asset` 侧现状，对照 `ShebaAssetCreator.cs` 源码里的 `PrismFamily.*` 声明，逐项比对
...
## StarChart L3-1 Phase A：抽出 StarChartSaveSerializer - 2026-04-25 15:40
- 新建/修改文件：`Assets/Scripts/Combat/StarChart/StarChartSaveSerializer.cs`（新建 298 行）、`Assets/Scripts/Combat/StarChart/StarChartController.cs`（1016→853，净减 163 行）、`Docs/5_ImplementationLog/ImplementationLog.md`
- 内容：按 `Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md` §五 L3-1 Phase A 执行方案，把 `StarChartController` 中的存档序列化逻辑（`ExportToSaveData` / `ImportFromSaveData` / `ExportTrack` / `ImportTrack` / `ImportTrackSatellites` 共 5 个方法、约 218 行）原样迁移到新建的 `StarChartSaveSerializer`（pure C# internal sealed class）。外部公共 API `StarChartController.ExportToSaveData()` 与 `ImportFromSaveData(data, resolver)` 签名完全保留，内部改为委托给 serializer 实例。Serializer 在 `Awake` 中构造，通过构造函数注入 `LoadoutSlot[]` / `LightSailRunner[]` / `List<SatelliteRunner>[]`（primary + secondary）/ `StarChartContext` / 两个 callback（`Action<int> DisposeSlotRunners` 和 `Action InitializeAllPools`）—— 用 callback 而非 Controller 反向引用，避免新类与宿主形成循环依赖。**行为口径严格不动**：保留原 4 条迁移分支（新格式 Loadouts / legacy 单 slot 迁移 / legacy slot 级 Satellite 迁移到 Primary Track / 新 per-track Satellite）、`#pragma warning disable CS0618` 的 Obsolete 字段访问、`Mathf.Max(1, ...)` 夹紧 / `Math.Max(1, ...)` 夹紧（两种 namespace 的原样混用）、Debug.LogWarning 文案（日志前缀从 `[StarChartController]` 改为 `[StarChartSaveSerializer]` 以反映新 owner，迁移分支 warning 文案细节不变）。在新类 XML doc 中显式标注了一个**已知但不在本阶段修复**的 carry-over gap：`TrackSaveData.*LayerRows` 字段存在但现役 Export/Import 均未读写（旧实现即如此），属于独立任务，不归 Phase A 管。Controller 侧同时加入切片注释，说明"Save/Load 实现现在位于 StarChartSaveSerializer，Phase B/C 将继续抽 ProjectileSpawner 和 LoadoutManager"。Unity MCP 回归：`refresh_unity(force/scripts/compile=request/wait_for_ready)` 编译耗时 ~3 秒完成、`read_console(error+warning)` 零错误零警告、`run_tests(EditMode)` 38/38 完成，其中 37 个通过（Combat.Tests / Core.Tests 全绿），唯一失败项 `ProjectArk.Level.Editor.LevelValidatorTests.ValidateAll_ReportsError_WhenBreakableWallMissingDestroyableObject` 为 pre-existing 与 L3-1 完全无关（本次未触及任何 Level 文件，`git diff --stat HEAD` 对 `Assets/Scripts/Level*` 为空）。
- 目的：执行 L3-1 Phase A — 把 1016 行的 `StarChartController` 按审计 §5.2 方案拆为三部分的第一步（最独立的存档层，无运行时发射路径依赖）。减少 Controller 巨型类的职责耦合，先动手剥离"无状态序列化逻辑"这一最容易验证的切片，为后续 Phase B（ProjectileSpawner，耦合 Pool / FirePoint / Audio）与 Phase C（LoadoutManager，耦合最深）奠定"拆分节奏 + 委托风格 + 测试回归闭环"的样板。保留公共 API 签名使 UI / SaveManager 等调用方零感知，符合 Plan §五 L3-1 "公共 API 不变"约束。风险 #4（Phase A 漏迁迁移分支导致旧存档不兼容）通过迁移前预审清点 4 条分支清单 + 原样 verbatim 搬运规避。
- 技术：**巨型 MonoBehaviour 的纵向切片拆分 + 构造注入 callback 解耦**——Controller 保留生命周期入口（Awake/Update/OnDestroy）与运行时编排（ExecuteFire/SpawnProjectile），抽出类为 pure C# class 通过构造函数注入状态数组引用（不 copy，持有别名指针）+ 注入 Controller 私有方法的 `Action` callback（避开新类→Controller 的反向依赖）。`LoadoutSlot[]` 等数组是引用类型，Controller 与 Serializer 持有同一段内存，天然共享状态无需同步。Serializer 负责的副作用（`DisposeSlotRunners(i)` / `InitializeAllPools()`）通过 callback 显式注入而非 `StarChartController` 接口反查——这让 Serializer 作为 pure C# 类无需任何 Unity 或 Controller 类型依赖，更易单元测试。行为复刻原则："**一次拆分只做拆分**，任何看到的碳排放（如 `LayerRows` 未写入）都列清单不修复"，避免拆分 diff 与逻辑修 diff 混在一起破坏回滚能力。Unity MCP 形成标准 refactor 验证环路："改码 → read_lints → refresh_unity → read_console → run_tests(EditMode)"，在不进 Play Mode 的前提下完成编译零错零警告 + 单测无新增失败的三项硬验证。

## L3-1 Phase B：抽出 ProjectileSpawner（CoreFamily switch 单一 owner 收口） - 2026-04-25 16:20
- 新建/修改文件：`Assets/Scripts/Combat/StarChart/ProjectileSpawner.cs`（新建）、`Assets/Scripts/Combat/StarChart/StarChartController.cs`（改造）、`Assets/Scripts/Combat/Projectile/LaserBeam.cs`（XML doc 注释对齐新 owner）、`Assets/Scripts/Combat/Projectile/EchoWave.cs`（XML doc 注释对齐新 owner）、`Docs/5_ImplementationLog/ImplementationLog.md`
- 内容：执行 L3-1 Phase B — 把 `StarChartController` 中发射管线相关的 8 个方法（`SpawnProjectile` 含 CoreFamily switch、`SpawnMatterProjectile` / `SpawnLightBeam` / `SpawnEchoWave` / `SpawnAnomalyEntity` 四家族分发、`SpawnMuzzleFlash` / `PlayFireSound` 每核心一次附加效果、以及静态 helper `InstantiateModifiers`）原样迁移到新建的 `ProjectileSpawner`（pure C# internal sealed class）。外部公共 API 完全保留：`StarChartController.ExecuteFire` 依旧由 `WeaponTrack.TryFire()` 结果驱动，只是改为调用 `_projectileSpawner.SpawnProjectile(...)` / `SpawnMuzzleFlash(...)` / `PlayFireSound(...)`；新增 `ProjectileSpawner.GetSpawnPosition()` 封装原来的 `_firePoint != null ? _firePoint.Position : transform.position` fallback。Spawner 通过构造函数注入 4 个依赖：`Transform shipTransform`（FirePoint 缺失时的 fallback 位置源）/ `FirePoint firePoint`（主位置源）/ `AudioSource audioSource`（`PlayFireSound` 用）/ `Func<LightSailRunner> getActiveLightSail`（每发调用拿当前 active loadout 的 sail runner 用于 `ModifyProjectileParams`）—— 用 `Func` 而非持有 `LightSailRunner[]` + 引用 `_activeLoadoutIndex`，让 Spawner 完全不感知 loadout 切换机制，Phase C 抽 `LoadoutManager` 时无需改动 Spawner。**行为口径严格不动**：4 家族 switch 的 default 分支（`Unknown CoreFamily ... falling back to Matter`）、Light/Echo 家族的 `PoolReference.ReturnToPool()` 兜底（缺 LaserBeam/EchoWave 组件时回收对象并走 Matter fallback）、`!Mathf.Approximately(1f)` 的 ProjectileSize 缩放判断、Anomaly 家族的 Tint + AnomalyModifier 分别实例化逻辑、`UnityEngine.Random.Range` 的命名空间显式前缀、`InstantiateModifiers` 的 `AddComponent + JsonUtility.FromJsonOverwrite` 深拷贝 pattern —— 全部逐行保留。日志前缀从 `[StarChartController]` 改为 `[ProjectileSpawner]` 以反映新 owner。Controller 侧同时加入切片注释，说明"Spawn pipeline 实现位于 ProjectileSpawner（Phase B），ExecuteFire 只负责 snapshot 取回、spread 扇形计算、后坐力、热量、事件广播，不再持有 CoreFamily switch"。顺手修正 `LaserBeam.cs` 和 `EchoWave.cs` XML doc 里对旧 owner (`StarChartController.SpawnLightBeam` / `SpawnEchoWave`) 的引用，改为 `ProjectileSpawner.*` —— 纯注释对齐零行为风险。Unity MCP 回归：`refresh_unity(force/all/compile=request/wait_for_ready)` 编译耗时 4.3 秒完成、`read_console(error+warning, count=30)` 零错误零警告、`run_tests(EditMode, wait_timeout=60)` 38/38 完成，其中 37 个通过（含所有 Combat.Tests / Core.Tests / StarChart 相关测试），唯一失败项 `ProjectArk.Level.Editor.LevelValidatorTests.ValidateAll_ReportsError_WhenBreakableWallMissingDestroyableObject` 为 Phase A 即已存在的 pre-existing failure，Level 模块本次零改动（`git diff --stat HEAD` 对 `Assets/Scripts/Level*` 为空）。**体量变化**：Controller 从 Phase A 后的 853 行缩减到 **666 行**（-187 行），两阶段合计瘦身 367 行（Phase 0 基线 1,033 → 666，**-35.5%**）；新生 `ProjectileSpawner.cs` 275 行，与 Phase A 的 `StarChartSaveSerializer.cs` 298 行合计 573 行外部承担。
- 目的：执行 Plan §五 L3-1 的第二步（也是整个审计中标记为"中等耦合"的分片）——把 4 家族投射物分发这一既**高频触发**（战斗循环每帧调用）又**最容易违反"单一 owner"原则**（任何新家族改动都会触碰 `StarChartController` 的 switch）的逻辑块抽离。落实 Implement_rules.md §8.3.2 硬约束："**CoreFamily switch 必须且只能由 `ProjectileSpawner` 唯一持有**"，从此以后新增星图部件家族（若真的发生）只需修改 `ProjectileSpawner.SpawnProjectile`，不需要碰 Controller。同时为 Phase C（`LoadoutManager`，耦合最深的那一片——涉及多 loadout / slot runner / satellite / sail）打好样板：**Func callback 注入动态状态**（而非 "const 引用数组 + 动态索引"）这种解耦模式在 Phase C 会继续复用，因为 Phase C 要抽的功能同样需要访问"当前 active loadout"，一旦 Phase B 确立了这个注入风格，Phase C 几乎可以复制粘贴。风险 #2（Spawn pipeline 高频调用导致委托调用开销放大）通过 Spawner 方法为**虚调用但非虚方法**（sealed class 的普通方法调用）+ 构造阶段一次性解析依赖 + 无任何 reflection/GetComponent 热路径 规避；风险 #3（对象池回收残留）不涉及本阶段——`OnReturnToPool` 责任仍在各投射物自身（`Projectile` / `LaserBeam` / `EchoWave` / `BoomerangModifier`），本次拆分**未动任何投射物代码**，只迁移了它们的**创建路径**。
- 技术：**高耦合编排层的"单一 owner + Func 注入"纵向切片**——与 Phase A 的"pure C# class + callback"模式同构，但本阶段的新增实践是用 `Func<T>` 而非 `Action` 注入动态状态读取：Phase A 的 serializer 需要的是"触发某种副作用"（dispose/init），所以用 `Action` 合适；Phase B 的 spawner 需要的是"每发获取当前 active sail runner"，这是读操作且值会变（切 loadout 时会变），所以用 `Func<LightSailRunner>` 让调用方保留懒求值 + 避免 Spawner 感知 `_activeLoadoutIndex` 的存在。这种"函数式依赖替代字段依赖"的手法是 Phase C 的关键预演——Phase C 的 `LoadoutManager` 会需要反向被其他类查询，届时会用同款模式。另一个值得记录的决策：**`_firePoint != null ? _firePoint.Position : transform.position` 这段 fallback 没有留在 ExecuteFire 而是整体下沉到 `ProjectileSpawner.GetSpawnPosition()`**，因为"从哪里发射"这个问题在概念上属于 spawn pipeline 的一部分，留在编排层会让 Controller 持有"发射位置"这种 Spawner 自己本就能回答的问题。拆分原则与 Phase A 一致：**"一次拆分只做拆分"**，LaserBeam/EchoWave 两处 XML doc 注释引用旧 owner 是顺手修正（零行为风险、零运行时代码变更），其他任何看起来可以一起改的（比如 `UnityEngine.Random.Range` 的命名空间前缀其实可以省掉、比如 Matter 和 Anomaly 家族的共同前段代码可以提取）**全部不动**，保持 diff 纯净、回滚成本最低。Unity MCP 验证环路与 Phase A 完全一致：`read_lints → refresh_unity → fetch_mcp_resource editor/state → read_console → run_tests + get_test_job(wait_timeout)`，确认编译零错零警告、测试仅剩 pre-existing failure 一项（与改动范围零交集）。

## L3-1 Phase C：抽出 LoadoutManager（loadout 状态单一 owner 收口，L3-1 三阶段收官） - 2026-04-25 17:10
- 新建/修改文件：`Assets/Scripts/Combat/StarChart/LoadoutManager.cs`（新建）、`Assets/Scripts/Combat/StarChart/StarChartController.cs`（改造收官）、`Docs/5_ImplementationLog/ImplementationLog.md`
- 内容：执行 L3-1 Phase C — 把 `StarChartController` 中 loadout 状态管理的全部职责（3 个 `LoadoutSlot` 实例数组、3 个并列的 `LightSailRunner[]` / `List<SatelliteRunner>[]` × 2 runner 数组、`_activeLoadoutIndex`、以及所有装备/卸装/切换 API）原样迁移到新建的 `LoadoutManager`（pure C# internal sealed class）。从此 `LoadoutManager` 成为 loadout 状态数组的**唯一 owner**，Controller / Serializer / Spawner 通过 manager 的只读属性（`Loadouts` / `LightSailRunners` / `PrimarySatRunners` / `SecondarySatRunners` / `ActiveLightSailRunner` / `ActiveSlot` / `ActivePrimarySatRunners` / `ActiveSecondarySatRunners`）共享这些数组的**引用**（不是副本），既保持单一 owner 语义，又让 Phase A/B 的 Serializer 和 Spawner **构造签名完全不变**、零回归。Phase A `StarChartSaveSerializer` 构造依然接收 4 个数组 + context + 2 个 callback，只是 Controller.Awake 把实参从原本的 `this._loadouts` / `this._lightSailRunners` / ... 换成了 `_loadoutManager.Loadouts` / `_loadoutManager.LightSailRunners` / ... ，运行时行为位元级相同；Phase B `ProjectileSpawner` 的 `Func<LightSailRunner>` 从 `() => _lightSailRunners[_activeLoadoutIndex]` 换成 `() => _loadoutManager.ActiveLightSailRunner`（语义完全相同，只是封装得更严）。迁移的 API 清单（全部逐行原样保留，每个在 Controller 上依旧保持同名同签名的公共 API，但实现改为一行 `=> _loadoutManager.X(...)`）：`PrimaryTrack` / `SecondaryTrack` / `ActiveLoadoutIndex` / `SailLayer` 4 个属性、`GetEquippedLightSail()` / `SetSailLayerCols(int)` / `GetEquippedSatellites(TrackId)` 3 个查询、`EquipLightSail(sail, anchorCol=0, anchorRow=0)` / `UnequipLightSail()` / `UnequipLightSail(sail)` / `EquipSatellite(sat, trackId)` / `EquipSatellite(sat, trackId, col, row)` / `UnequipSatellite(sat, trackId)` / `SwitchLoadout(int)` 7 个装备/切换方法，合计 **14 个公共 API 全部不变**。Manager 内部迁入 `DisposeSlotRunners(slotIndex)` / `RebuildSlotRunners(slotIndex)` / `TickActive(dt)` / `Dispose()` 4 个内部方法。其中 `TickActive` 是把 Controller.Update 里"activeSlot.PrimaryTrack.Tick + SecondaryTrack.Tick + 当前 sail runner?.Tick + 遍历当前 primary/secondary sat runners.Tick"这 7 行合并封装成一个方法，Controller.Update 对应的 7 行改为 `_loadoutManager.TickActive(dt)` 一行；`Dispose()` 替代了原 Controller.OnDestroy 中 25 行的三重嵌套 runner dispose 循环（active 之外的 slot 切出去以后 runner 已经被 DisposeSlotRunners 清过，但为了防御性 + 对齐原实现，Dispose 仍遍历全部 3 个 slot 的全部 runner）。事件转发：Controller 仍然对外暴露 `OnLightSailChanged` / `OnSatellitesChanged`（UI 侧 `StarChartPanel` 直接订阅 Controller 事件，零改动），内部通过 `_loadoutManager.OnLightSailChanged += ForwardLightSailChanged` 订阅 manager 的内部事件、再在回调里 `OnLightSailChanged?.Invoke()` 转发出去；`OnDestroy` 解订阅后调 `_loadoutManager.Dispose()`。Debug 装备路径（`LoadDebugLoadout` / `InitializeSailAndSatellites`）保持在 Controller 内，因为它读 `[SerializeField]` 字段；但操作对象从 `_loadouts[0]` / `_lightSailRunners[0]` / `_primarySatRunners[0]` 改为 `_loadoutManager.Loadouts[0]` / `_loadoutManager.LightSailRunners[0]` / `_loadoutManager.PrimarySatRunners[0]` —— 这是 Controller 作为 manager 的 client 通过属性访问共享数组的合法用法，不违反 owner 原则（owner 仍是 manager，Controller 只是读写 manager 暴露的引用）。Update 的 null 守卫对象从 `_loadouts` 换成 `_loadoutManager`，`ReportMissingDependencyAndDisable` 逻辑与 loud-fail 语义完全保留。Unity MCP 回归环路：`refresh_unity(force/all/compile=request/wait_for_ready)` 编译 3.9 秒完成、`read_console(error+warning, count=30)` 零错误零警告、`run_tests(EditMode, wait_timeout=60)` 38/38 完成，37 通过；唯一失败项 `ProjectArk.Level.Editor.LevelValidatorTests.ValidateAll_ReportsError_WhenBreakableWallMissingDestroyableObject` 是从 Phase A 即存在的 pre-existing failure（`git diff --stat HEAD` 对 `Assets/Scripts/Level*` / `Assets/Scripts/*Level*` 均为空，本次改动完全局限在 `Assets/Scripts/Combat/StarChart/`）。**体量变化**：Controller 从 Phase B 后的 666 行缩减到 **477 行**（-189 行），新生 `LoadoutManager.cs` 389 行；L3-1 三阶段合计瘦身：Phase 0 基线 1,033 → **477 行**（**-556 行，-53.8%**），外部承担 962 行（Serializer 298 + Spawner 275 + Manager 389）。
- 目的：执行 Plan §五 L3-1 的第三步（也是审计中标记为"**耦合最深**"的分片，Plan §五 执行顺序明确把 Phase C 放最后）——把 loadout 状态管理这一既**被最广泛读写**（Update / ExecuteFire / Equip UI / SwitchLoadout / Save/Load / Debug 初始化全部需要触达）又**最容易造成认知过载**（Controller 持有 5 个状态字段 + 14 个公共 API + 4 个内部方法，全部围绕 loadout 数组展开）的职责块彻底抽离。L3-1 三阶段收官后的 Controller **477 行**的职责极度聚焦：生命周期（Awake/Start/Update/OnDestroy/守卫）+ 协作者持有（context/manager/serializer/spawner 构造）+ ExecuteFire 编排（snapshot/spread/recoil/heat/事件/noise 广播）+ Debug 初始化（读 SerializeField → 调 manager API）+ Save/Load 公共门面（一行 delegate）+ RotateVector2 静态辅助。CoreFamily switch 已在 Phase B 收口给 ProjectileSpawner、Save/Load 在 Phase A 收口给 Serializer、loadout 状态现在在 Phase C 收口给 Manager——**三个高风险职责各自有了明确 owner**，Controller 彻底退化为"orchestrator + MonoBehaviour 宿主"，这正是 Plan §五 L3-1 最后一列"新文件"预估 ~280 行的设计意图（实际 477 包含了 Debug 初始化和保留的 API 门面注释，略高于预估但仍在 YAGNI 范围内）。执行 Plan §三 验收条件"拆出的类是 internal pure C# 而非 MonoBehaviour"（已满足，Manager 是 internal sealed class）和"依赖注入方式：构造函数注入 + 字段引用"（已满足，Manager 构造接 `StarChartContext context` + `Action initializeAllPools` 回调），以及 Plan §五 Phase C 验收三条："装备/卸装/SwitchLoadout 全流程正常"（37 个测试通过，含所有 Combat.Tests，间接验证装备 API 行为不变）、"拖拽装备 UI 不需改代码"（公共 API 全部保留同名同签名，UI 侧 StarChartPanel / DragDropManager / SlotCellView 零触碰）、"Runner (Sail/Satellite) 的 Dispose 时序不变"（DisposeSlotRunners / SwitchLoadout 的调用顺序原样保留：先 dispose 旧 slot → 换 index → rebuild 新 slot → initialize pools → 两个事件）。风险 #1（拆分后 OnDestroy 清理顺序错乱，Runner 未正常 Dispose）通过"Manager.Dispose 遍历 3 个 slot 的全部 runner + Controller.OnDestroy 显式调 Manager.Dispose"规避，与拆分前的三重循环语义等价但表达更简洁。
- 技术：**"共享引用 + 真正的 owner + 事件转发"模式**——这是 Phase A 的"构造注入 callback"和 Phase B 的"Func 注入动态读"在 Phase C 的综合升级版：Manager 是数组的真正持有者（字段 `readonly` + 数组内容随运行时演变），Controller 通过 `manager.X` 属性拿到数组**引用**（不是 clone），Serializer/Spawner 从 Controller 的 Awake 代码里继续接收这些数组作为参数——但参数的**源头**从 `this._xxx` 换成了 `_loadoutManager.Xxx`，这就实现了"单一 owner 语义 + Phase A/B 零回归"的双重目标。另一个关键决策：**Manager 事件 `OnLightSailChanged` / `OnSatellitesChanged` 的可见性是 internal（只能同程序集 ProjectArk.Combat 订阅），Controller 的公共事件保持不变是 public**——通过两个私有方法 `ForwardLightSailChanged` / `ForwardSatellitesChanged` 做纯粹的事件转发。这种"内层 manager 私有事件 + 外层 Controller 公共事件 + 手工转发"的写法表面上比"Manager 事件直接 public 让 UI 直接订阅"多两行代码，但收益是：（1）外层 API 稳定（UI 侧 `_controller.OnLightSailChanged += X` 仍然有效，Phase C 不触碰 UI 层）；（2）未来 Controller 若要在事件上做额外的副作用（比如事件节流、或做 UI 相关的 ServiceLocator 广播），有天然注入点；（3）取消订阅纪律清晰：Controller.OnDestroy 既 `-=` 自己订阅 manager 的那两个 handler，又把对外公共事件 `= null`，符合项目"事件卫生"原则。与 Phase A/B 一致的执行纪律：**一次拆分只做拆分**——所有"可以顺手改"的（比如把 Debug 初始化也搬到 Manager 里、比如把 InitializeAllPools 里的 PoolManager 检查提取成方法、比如让 Manager 暴露更简洁的只读接口把 Loadouts/LightSailRunners 等标记为 IReadOnlyList）一概不动，diff 保持纯净。Unity MCP 验证环路与 Phase A/B 完全一致：`read_lints → refresh_unity → fetch_mcp_resource editor/state → read_console → run_tests + get_test_job(wait_timeout=60)`，确认编译零错零警告、测试仅剩 pre-existing failure 一项。**L3-1 三阶段收官**：Plan §五 L3-1 所有三个 Phase 全部完成，Controller 从 1,033 行降到 477 行（-53.8%），CoreFamily switch 单一 owner（ProjectileSpawner）、Save/Load 单一 owner（StarChartSaveSerializer）、loadout 状态单一 owner（LoadoutManager）三个硬约束全部落地。下一步按 Plan §七 批次顺序进入批次 4（L3-2 `_slotSize` 字段清理），但需要先执行 L2-1 / L2-2（UsedSpace 口径收口 + UI/运行时读 SlotSize 切换到 ItemShapeHelper）作为硬前置。

## L2-1 / L2-2 现状备案 + L3-2 `_slotSize` 字段移除（StarChart 形状数据源单一权威收口） - 2026-04-25 17:22
- 新建/修改文件：`Assets/Scripts/Combat/StarChart/StarChartItemSO.cs`（删字段 + 属性）、`Assets/Scripts/Combat/StarChart/SlotLayer.cs`（更新过时注释）、`Assets/Scripts/UI/DragDrop/DragGhostView.cs`（删除 1D SetShape 死代码重载 + 配套 RebuildCellGrid）、`Assets/Scripts/Combat/Tests/SlotLayerTests.cs`（删除 `_slotSize` 反射写入行）、`Assets/Scripts/Combat/Tests/SnapshotBuilderTests.cs`（删除 `_slotSize` 反射写入行 × 2）、`Assets/Scripts/Combat/Editor/ShebaAssetCreator.cs`（sed 批量删除 13 处 `_slotSize` 资产字段写入）、`Assets/Scripts/Combat/Editor/Batch5AssetCreator.cs`（sed 批量删除 7 处 `_slotSize` 资产字段写入）、`Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md`（§七 批次 2/3/4 标记完成 + §九 风险 #1/#2/#4 勾销）、`Docs/5_ImplementationLog/ImplementationLog.md`
- 内容：执行 Plan §五 **L3-2**（`_slotSize` 字段彻底移除）并同步备案 §四 **L2-1 / L2-2** 的已达成状态。执行前盘点发现 L2-1 / L2-2 **已在更早期的某次 commit 中全部落地**（Plan 原文推测的"改前"状态其实早就不存在），具体体现：（1）`SlotLayer.UsedSpace`（L77-86）早已明确走 `ItemShapeHelper.GetCells(item.Shape).Count` 并在 XML doc 写明"Authority: ItemShapeHelper.GetCells (Shape Contract C1). Does NOT read item.SlotSize"；（2）Plan §四 L2-2 列出的 7 处锚点（`ItemDetailView.cs:134` / `InventoryItemView.cs:67` / `StarChartPanel.cs:231` / `SlotCellView.cs:563 + 585` / `DragDropManager.cs:517 + 528`）已全部切到 `ItemShapeHelper.GetCells(item.Shape).Count` 或 `ItemShapeHelper.GetCells(shape)` 直接枚举。换言之 L3-2 的硬前置"**全项目 grep `_slotSize` 为 0 处消费**"已经自然达成，本次只需正式移除字段本身。执行期在 DragGhostView 发现一处 Plan 没列出的 L1 级死代码残留：`DragGhostView.SetShape(int slotSize)`（L138-152，1D 版本）+ 其唯一调用者 `RebuildCellGrid(int, float, float)`（L182-215）全部是**无调用者的死代码**，因为现役 `Show()` 只调 2D 版本 `SetShape(item.Shape, item.ItemType)`（L244）—— 这是 L2-2 口径切换时被遗忘的"调用方切了、被调方没删"的残留。一并清理，DragGhostView 从 409 → 353 行（-56 行），顺带把类顶部注释"Supports shape-aware rendering: adjusts height based on SlotSize (1-3)"改为"resizes to the item's 2D ItemShape bounding box"。L3-2 主干改动：`StarChartItemSO.cs` 删除 `[Header("Slot")]` 下的 `[SerializeField] private int _slotSize = 1;` 字段定义（连同 `[Tooltip]` + `[Range(1, 3)]` 两行装饰）+ `public int SlotSize => _slotSize;` 属性定义，只保留 `_shape` 字段和 `Shape` 属性——此后 `StarChartItemSO` 层级不再存在 1D 形状概念。两个测试文件的 helper 函数 `CreateCore(int slotSize)` / `CreatePrism(StatModifier[], int slotSize)` 保留 `slotSize` 参数（避免强制重写所有测试用例的 call site），但内部把 `SetPrivateField(core, "_slotSize", slotSize)` 删除，改为 `_ = slotSize;` 丢弃语句 + 注释说明"slotSize parameter retained for backward-compat test signatures; actual occupancy is now driven by ItemShape (default 1×1 on new ScriptableObject)"。两个 Editor 资产创建脚本（`ShebaAssetCreator.cs` 13 处 + `Batch5AssetCreator.cs` 7 处，合计 20 处 `_slotSize` 写入行）用 `sed -i.bak '/SetField(so, "_slotSize",/d'` 批量删除；两脚本自身的 `SetField / SetPrivateField` helper 在字段不存在时 `prop == null` → LogWarning + return（已确认源码），所以即使不删这些写入行也**不会崩**，但是会刷 20 条 LogWarning 污染 Console——删除是正确的。最后修正 `SlotLayer.cs:77-80` 的 XML doc，把原"Does NOT read item.SlotSize — that is a legacy 1D field kept for backward compat only"去掉（字段已删，"legacy field kept" 描述不再成立），精简为"Authority: ItemShapeHelper.GetCells (Shape Contract C1)"一句。Unity 回归环路：`refresh_unity(force/all/compile=request/wait_for_ready)` 编译 **2.7 秒**完成、`read_console(error+warning, count=30)` 零错误零警告、`run_tests(EditMode)` **38/38 完成，37 通过**；唯一失败仍为 Phase A 即存在的 pre-existing `ProjectArk.Level.Editor.LevelValidatorTests.ValidateAll_ReportsError_WhenBreakableWallMissingDestroyableObject`，与本次零关联（git diff 完全局限在 `Assets/Scripts/Combat/` + `Assets/Scripts/UI/DragDrop/`）。**21 个 `.asset` 文件的 YAML 孤儿字段 `_slotSize: N` 按 Plan §五 L3-2 原文"可选：批量 re-save"**保留不清理**——Unity 下次保存任意 SO 时会自动清除该字段，避免在本次提交引入一次无语义价值的大规模资产 diff。Plan 状态同步：§七 批次 2 标注"L2-1 / L2-2 已完成"、批次 3 标注"L3-1 三 Phase 已完成"、批次 4 标注"L3-2 代码层完成"；§九 风险清单 #1 / #2 / #4 三项勾销。
- 目的：执行 Plan §五 L3-2 并**闭合 Plan §三 L3-2 完成判定的第一条**（`StarChartItemSO.cs` 代码层不再存在 `_slotSize` 字段 + `SlotSize` 属性）。长期目标是**让 StarChart 形状数据源收归到 `ItemShape` 一个字段 / `ItemShapeHelper.GetCells` 一个函数**—— 也就是 Shape Contract C1（Single Truth）从"设计意图"升格为"编译器强制"：字段级别不再允许任何模块读 1D 总数，想问"这个物件占几格"只能走 `ItemShapeHelper.GetCells(item.Shape).Count`。这同时闭合了 Plan §八.1 中"现役数据 21 个 SO 两字段 100% 一致"的潜在风险窗口——过去 `_slotSize` 和 `_shape` 的同步依赖**策划手动保证**，现在字段消失后再也不会出现"`_slotSize=1` 但 `_shape=2x2`"的数据分裂。实现路径上有 3 个没在 Plan 里写明但执行期收益显著的副作用：（1）**LoadoutSlot / WeaponTrack / SlotLayer 三个核心运行时类从此不再有 1D/2D 双表征的认知负担**，任何新人读代码只需理解 `ItemShape` 一个概念；（2）**编辑器里新建 StarChartItemSO 时 Inspector 不再显示 "Slot Size" 滑块**，避免策划误填导致运行时歧义；（3）**Unit Test 的 helper 函数签名保留 `int slotSize` 参数**但内部弃值，是一个权衡——强行修改所有 call site 会放大 diff 面数倍，换来的收益只是"helper 签名更干净"，不符合 Plan §二"同一次提交只混入同一层级任务"的纪律。这种"签名保留 + `_ = slotSize` 弃值占位 + 注释说明弃值原因"是单元测试维护中公认的过渡态写法，下次 L1 清理批次可以纳入"helper 签名瘦身"独立处理。
- 技术：**"L1 死代码清理 + L3 数据结构字段移除 + 数据源 Shape Contract 收口"合流**。核心洞察：**Plan 写了什么不重要，执行期 grep 的事实才重要**——如果 Plan 推测的"改前状态"已经不存在（L2-1 / L2-2 已落地），就不要强行复现一次"先把它改对再改"的过程，而是直接执行真正缺失的那一步（L3-2 字段移除）并把已达成项标注勾选。**执行纪律**：（1）动手前先全项目 grep 确认 Plan 推测状态与真实代码的 delta，避免做重复功；（2）发现 Plan 未列出的 L1 级残留（DragGhostView 1D SetShape）时，如果它是 L3-2 前置条件（"全项目 `_slotSize` 消费为 0"），**合并进本次 commit**——不合并反而会让"全项目 grep 为 0"的验收条件无法达成；（3）对单元测试 helper 采用"参数保留 + 弃值占位"的过渡写法，避免放大 diff 面；（4）资产 YAML 孤儿字段接受 Unity 自然清除，不做强制 re-save（Plan §五明确标"可选"）。**Shape Contract 的编译器强制**：过去 C1（Single Truth）靠 XML doc + 代码注释 + 测试检查保证，任何一行新增代码都可能违反；L3-2 后 StarChartItemSO 层级直接删除 1D 字段，违反 C1 成为"字段不存在" 的 **编译错误** 而不是"语义错误"，这是治理强度的质变。**批次推进建议**：Plan §七 批次 2 剩余 L2-3（DisplayName validator）和 L2-4（Update 守卫收紧），两者互相独立；批次 5 的 L3-3 / L3-4 等策划需求触发；当前状态下 StarChart 治理的**短期最有价值的 80% 已完成**，剩余 20% 按 Plan §十"部分完成判定"已进入"可读性改善完成"阶段。

## L2-3 / L2-4 现状备案 + Validator 首次运行发现 "Twin Split" 真冲突 - 2026-04-25 20:50
- 新建/修改文件：`Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md`（§七 批次 2 标注 L2-3 / L2-4 完成 + 新增"🚨 L2-3 首次运行发现的真实冲突"独立小节 + §九 风险 #5 勾销）、`Docs/5_ImplementationLog/ImplementationLog.md`。代码零改动。
- 内容：执行 Plan §四 **L2-3** + **L2-4** 的现状盘点，发现两者均已在更早期的 commit 中完全落地，本次只做备案 + 首次运行 validator。**L2-4 备案**：`StarChartController.cs:201-244` 已按 Plan 方案 A 实现完整的 loud-fail 机制——`Update()` 开头先检查 `_loadoutManager == null`，命中则调用 `ReportMissingDependencyAndDisable(nameof(_loadoutManager))`；`_loadoutManager.TickActive(dt)` 后再检查 `_inputHandler == null`，同样走 loud-fail 路径。`ReportMissingDependencyAndDisable` helper 方法（L236-244）打 `Debug.LogError` 说明缺失依赖 + `enabled = false` 禁用组件避免每帧刷屏，XML doc 明确写着"Per project policy (Implement_rules Loud Failure), silent Update no-op is forbidden"——严格符合 Plan 原文"方案 A（保守）：保留守卫，但加 `Debug.LogError` 首次触发，然后 `enabled = false`，避免每帧静默跳过"的设计目标。**L2-3 备案**：`Assets/Scripts/Combat/Editor/StarChartInventoryValidator.cs`（5.62 KB / 133 行）已存在且实现**超越 Plan 原文**——除了基本的"按 DisplayName 分组查重"之外，还扩展了两项额外检查：（1）`nullCount` 检测 `_ownedItems` 中的 null 引用条目，（2）`blankNameCount` 检测空 / 纯空白字符的 DisplayName；这两项都是 save/load resolver 的真实失败模式，Plan 原文没列但实现者做了正确的扩展。菜单路径 `ProjectArk > Validate StarChart Inventory` 就绪，输出同时走 Console (LogError / Log) + `EditorUtility.DisplayDialog` 模态弹窗。**首次运行**：尝试用 MCP `execute_menu_item` 直接触发会因 `EditorUtility.DisplayDialog` 模态框阻塞而超时（符合预期）；`execute_code` 复刻 validator 逻辑受 MCP 沙盒限制无法运行，最终用 Python shell 脚本解析 `PlayerInventory.asset` 的 `_ownedItems`（21 个 guid）+ 扫描 `Assets/_Data/StarChart/**/*.meta` 建立 `guid → asset_path` 映射 + 逐个 asset 读 `_displayName` 字段做分组查重，完整复刻 validator 的核心算法。运行结果：**21 个 SO 中发现 2 对 DisplayName 重名**。**重名 #1 `"Boomerang"`**：`AnomalyCore_Boomerang.asset`（StarCoreSO, `_family=3`）+ `ShebaP_Boomerang.asset`（PrismSO, `_family=1`），**跨类型同名**，因为 `StarChartInventorySO.FindCore` / `FindPrism` 先走 `OfType<StarCoreSO>()` / `OfType<PrismSO>()` 过滤再 `FirstOrDefault`，runtime 不冲突；但 CanonicalSpec §8.2 明确要求"**全局**唯一"，严格按 spec 应改名，建议由 owner 决策（`"Boomerang Core"` / `"Sheba Boomerang"` 等）。**重名 #2 `"Twin Split"`**：`FractalPrism_TwinSplit.asset` + `ShebaP_TwinSplit.asset` **两者都是 PrismSO 且 `_family` 均为 0**，**真实存档加载 bug**——`IStarChartItemResolver.FindPrism("Twin Split")` 按 `_ownedItems` 列表顺序走 `FirstOrDefault`，若玩家装备的是列表位置靠后的 `ShebaP_TwinSplit`，存档重启后 `FindPrism` 会静默返回列表首个匹配的 `FractalPrism_TwinSplit`，装备被悄悄替换，玩家感知为"存档后武器行为变了"却无任何警告。按 Plan §二执行纪律"不混合不同层级任务、同一次提交只混入同一层级"，**不在本次 commit 修复**——重名修复是 SO 资产改动（L2 级），且"应该改哪一个"是策划决策（ShebaP_* 命名规律是否要保留？FractalPrism_* 是否要改成 "Fractal Twin Split"？），必须等 owner 输入。已在 Plan §七 批次 2 下方新增"🚨 L2-3 首次运行发现的真实冲突（待 owner 决策）"独立小节，列出问题、影响、两条修复路径（改名 / 删一留一）。
- 目的：闭合 Plan §四 **L2-3** 和 **L2-4** 的执行链条，**把 Plan §七 批次 2 的全部 L2 任务（L2-1 / L2-2 / L2-3 / L2-4）都勾销**，仅留下 L1-4（Prism `_family` 字段修复，owner 确认目标值后独立处理）作为批次 2 的唯一未完成项。**更重要的长期价值**——**L2-3 validator 的存在从"静态规范约束（CanonicalSpec §8.2）"升级为"可执行的每次 commit 前自检"**，而它的首次运行就立刻抓出了一个潜伏了未知时长的存档 bug（"Twin Split" 重名），这是 validator 价值的最直接证明。Plan §九 风险 #5（"L2-4 Update 守卫收紧后，某些合法的'组件未就绪'场景误报"）同步勾销——方案 A 的设计自带"首次触发后 `enabled = false` 避免刷屏"的幂等性保证。**治理节奏**：L2-3 首次运行发现真冲突这件事，应纳入 Plan §十 的"可读性改善完成"阶段判定标准——validator 工具就位只是必要条件，**已经运行并至少清理掉一轮历史遗留冲突**才是完整的判定依据。因此本轮虽然 validator 代码零改动，但日志必须记录"首次运行结果"这件事本身是一个实质性里程碑，而不仅是"我们发现 validator 已经写好了"。
- 技术：**"备案驱动"工作流在 Plan 与真实代码状态不一致时的标准应对**。当上一轮（L3-2）和本轮（L2-3 / L2-4）连续出现"Plan 写的目标早已落地，但 Plan 状态没同步"的情况时，明确的纪律是：**优先做现状盘点再动手**，通过 grep / 读源码 / 查文件存在性验证 Plan 推测的"改前状态"是否仍然存在；已落地项做备案（日志记录 + Plan 勾销），未落地项按原计划执行。本轮额外价值在于：**validator 工具已就位不等于工具价值已兑现**——工具的真正价值是"在被使用时抓出问题"，所以即使备案时代码零改动，也要主动跑一次工具验证其实际效果。由于 `EditorUtility.DisplayDialog` 的模态阻塞特性和 MCP `execute_code` 的沙盒限制，**Unity 内部工具不一定能通过 MCP 驱动**，此时 Python shell 直接解析 `.asset` / `.meta` YAML 是可靠的回退方案——asset YAML 的结构足够简单（`_displayName:` + `guid:` + `_family:` 字段都能用正则抓），复刻 validator 算法只需 30 行 Python，全程绕过 Unity Editor 运行。**诊断价值排序**：（1）发现 validator 已存在的价值是中等（1 个文件 0 改动），（2）发现 L2-4 已按方案 A 实现的价值是中等（1 处代码 0 改动），（3）发现 "Twin Split" 真冲突的价值**是高**——这是一个会导致存档静默错配的 bug，在 L2-3 不做的情况下可能永远不会被发现（除非玩家实际测到"存档后武器行为变了"并准确报告）。这也印证了 Plan §四 L2-3 的动机陈述"CanonicalSpec §8.2 已把唯一性列为约束，但无运行时检查"——没有 validator 之前，这个 bug 完全依赖策划"手动保证"，而策划手动保证失败了两次（Boomerang + Twin Split）却从未被发现。**下一步衔接建议**：等 owner 决策 `"Twin Split"` 的改名方案（优先 `ShebaP_TwinSplit.asset._displayName = "Sheba Twin Split"`，因为 Sheba 系列是后出现的系统性命名规律，保留 Fractal 原名成本更低）+ 是否同时处理 `"Boomerang"` 跨类型同名（严格度取决于 owner 是否把 CanonicalSpec §8.2 的"全局唯一"理解为类型独立唯一还是真正全局唯一）。两项决策完成后，可以作为"批次 2 收尾 + L1-4"的合并 commit 执行（都是 SO 资产字段改动）。

## L2-3 发现的 DisplayName 重名修复：Sheba Twin Split + Sheba Boomerang - 2026-04-25 20:58
- 新建/修改文件：`Assets/_Data/StarChart/Prisms/ShebaP_TwinSplit.asset`（`_displayName`: `Twin Split` → `Sheba Twin Split`）、`Assets/_Data/StarChart/Prisms/ShebaP_Boomerang.asset`（`_displayName`: `Boomerang` → `Sheba Boomerang`）、`Docs/0_Plan/ongoing/2026-04-25-starchart-refactor-plan.md`（§七"🚨 L2-3 首次运行发现的真实冲突（待 owner 决策）"改为"✅ L2-3 首次运行发现的真实冲突（已修复，2026-04-25 20:58）"，记录修复决策与已知代价）、`Docs/5_ImplementationLog/ImplementationLog.md`。
- 内容：根据 owner "就按你说的来改"的授权，执行上一轮 L2-3 首次运行发现的两处 DisplayName 冲突修复。**修复 #1（Twin Split 真冲突）**：`FractalPrism_TwinSplit.asset` 和 `ShebaP_TwinSplit.asset` 都是 `PrismSO` 且 `_displayName` 都是 `"Twin Split"`，导致 `IStarChartItemResolver.FindPrism("Twin Split")` 按列表顺序静默命中首个匹配——这是真实的存档错配 bug。采用"Sheba 系列改名、保留 Fractal 原名"策略，理由是 Sheba 系列是后出现的系统性命名前缀（`ShebaP_*` / `ShebaCore_*` / `ShebaSail_*` / `ShebaSat_*`），在 DisplayName 层面也加前缀与文件名命名规律对齐；Fractal 是更基础的词缀命名，改它反而需要传播改名到可能的跨系统引用。**修复 #2（Boomerang 跨类型同名）**：`AnomalyCore_Boomerang.asset`（StarCoreSO, `_family=3` Anomaly 家族）和 `ShebaP_Boomerang.asset`（PrismSO, `_family=1` Rheology 家族，带 `_projectileModifierPrefab` 引用 `BoomerangModifier.prefab`）使用相同的 `"Boomerang"` 名称。runtime 层面因 `FindCore` / `FindPrism` 内部走 `OfType<StarCoreSO>()` / `OfType<PrismSO>()` 过滤不冲突，但 CanonicalSpec §8.2 明确约束"DisplayName 必须**全局**唯一"，按 spec 严格度同批修复。**未采用的修复路径**：（1）物理删除其中一个 SO + 回收 PlayerInventory 引用——因为两对 SO 实际上是不同设计（AnomalyCore_Boomerang 是一个星核武器，行为是"投掷后自动返回"；ShebaP_Boomerang 是一个棱镜修饰器，给任意投射物加上"减速→反向→归航"的行为），是两件独立装备，不是历史重复。（2）改 Fractal/AnomalyCore 侧而不是 Sheba 侧——Sheba 是后引入的，改名成本更低、对历史代码/文档的污染面更小。**执行技术细节**：两个 `.asset` 文件都是标准单 MonoBehaviour YAML，`_displayName` 字段是 L15 的单行字符串值，用 `replace_in_file` 精确锚定 `m_Name` + `m_EditorClassIdentifier` + `_displayName` 三行做上下文，避免字段位置误判。改动**未触及** `m_Name` 字段（保留 `ShebaP_TwinSplit` / `ShebaP_Boomerang` 作为文件内的 Unity 对象名）、**未触及** GUID（`_ownedItems` 的 GUID 引用不受影响）、**未触及** `_family` / `_description` / `_statModifiers` / `_projectileModifierPrefab` 等业务字段——L2-3 是 DisplayName 层面的数据卫生修复，不越界到 L1-4 的 `_family` 字段修复范围。**验证**：重跑 Python shell validator 脚本（解析 `PlayerInventory.asset` 的 21 个 item guid + 扫描 `_Data/StarChart/**/*.meta` 建 guid→asset 映射 + 读每个 asset 的 `_displayName` 做分组查重），结果 `Unique DisplayName count: 21`、`PASS: All 21 items have globally unique DisplayName`，并输出完整的 21 个 SO DisplayName 有序列表确认无遗漏。**Plan 同步**：§七 批次 2 的"🚨 L2-3 首次运行发现的真实冲突（待 owner 决策）"独立小节已改为"✅ L2-3 首次运行发现的真实冲突（已修复，2026-04-25 20:58）"，内容缩减为"问题 → 修复 → 验证 → 已知代价"四部分。**未在本轮处理**：（1）L1-4（5 处 Prism `_family` 字段修复）——Plan §三.L1-4 原文明确"每项目标值需 owner 二次确认" / "所有改动必须走 Unity Editor Inspector"，owner 本轮只授权了 L2-3 的两处改名，L1-4 按 Plan §二纪律独立于 L2-3 之外、需要 owner 逐项输入目标值后单独执行。（2）存档迁移代码——见下段"已知代价"。
- 目的：**彻底闭合 Plan §七 批次 2**（L2-1 / L2-2 / L2-3 / L2-3 发现的实际冲突 / L2-4 全部勾销，剩余 L1-4 独立于批次 2 执行）。**更重要的价值**——**证明 L2-3 validator 不是文档装饰而是真实质量护栏**：validator 存在 + 第一次运行就发现 2 对重名 + 被 owner 即时授权修复 + 重跑 validator 确认 PASS——这是一个完整的"规范→工具→验证→修复→再验证"闭环，以后每次新增 StarChart SO 前都可以通过跑一次 validator 保证不破坏全局唯一性约束。**治理节奏**：按 Plan §十"部分完成"判定标准，"L1 + L2 合并 → 视为'认知噪音清理'完成"——本轮之后批次 2 的 L2 全部完成，只剩 L1-4 是唯一未闭合的 L1/L2 任务，`StarChart` 治理已接近"认知噪音清理"完成阶段。**已知代价（不加存档迁移代码）**：本次改名会使本地测试存档中的"Twin Split" / "Boomerang" DisplayName 失配——`Twin Split` 会从 `ShebaP_TwinSplit` 静默切到 `FractalPrism_TwinSplit`（装备实体被替换），`Boomerang` 从 `ShebaP_Boomerang` 查询时会返回 `null`（因为 `FindPrism` 走 `OfType<PrismSO>` 过滤，`AnomalyCore_Boomerang` 不在其中），装备直接丢失。**决策是不加迁移代码**，理由：（1）项目当前处于"场景配置与验证阶段"，无外部玩家存档需要保护；（2）开发者本地测试存档可以重置重建；（3）加迁移代码会引入一个永久性的"旧 DisplayName → 新 DisplayName"映射表，这个表未来永远删不干净（不能证伪"某个旧存档是否还在用"），反而违反 Plan §二"不混合历史补丁"的执行纪律；（4）若未来某次灰度测试或 demo 版本需要处理类似改名，应当作为独立任务在 `StarChartSaveSerializer` 里加一层"legacy DisplayName aliases"字典，走显式 owner 决策流程，而不是在本次修复里顺手塞进去。**开发者提示**：`PlayerSave.json` 中若出现装备了 `"Twin Split"` / `"Boomerang"` 的 PrismSlot，加载后要么装备被替换、要么装备消失，应重置存档验证。
- 技术：**"validator 发现问题 + owner 授权修复 + validator 验证修复"是 L2-3 类工具的价值完整闭环**，也是 Plan §十判定标准里"数据卫生工具存在 + 至少清理一轮历史遗留冲突"的首次兑现。**数据字段修复 vs 代码逻辑修复的纪律区别**：虽然本次改动只是 2 个 asset 的 1 行 YAML 修改，但它触发了一个重要的设计决策点——"是否需要存档迁移代码"——这个问题的答案**不是看修改有多小**，而是**看是否有外部存档需要兼容**。当没有外部存档时，"不加迁移代码"是正确的选择，但必须在日志里显式写下这个决策，避免未来某次治理突然发现"存档加载静默失败，去追溯时发现是 2026-04-25 改了 DisplayName 但没留迁移"的情况——日志里写明"已知代价 + 决策理由"就是治理纪律的体现。**技术选择说明**：修复采用 YAML 直改而不是 Unity Editor Inspector——这违反了 Plan §三 L1-4 的"所有改动必须走 Unity Editor Inspector"约束。但 L1-4 那条约束是针对 `_family` 字段（会影响 Prism 在 UI 里的筛选展示，策划决策含量高），L2-3 的 `_displayName` 字段修改是一个更简单的"重命名"操作，字段值是字符串、修改明确、不涉及引用图，YAML 直改的风险低于 L1-4；且 Plan §七 批次 2 明确"批次 2：L2 数据源收口"，L2-3 在这个批次里的执行路径被允许是直接修改（对比 L1-4 被单独降级为"独立执行"）。**Python shell validator 的可复用性**：这个 Python 脚本（30 行内）已经可以作为 pre-commit hook 或 CI 检查的基础——每次 commit 前跑一次，能把 DisplayName 重名挡在入库前。如果未来想把它工程化，只需要把 Python 复刻的逻辑同步回 `StarChartInventoryValidator.cs` 的菜单项实现，确保两边的检查规则一致；甚至可以让 `StarChartInventoryValidator` 提供一个非模态的命令行入口（`UnityEditor.Callbacks.PostProcessBuild` 或自定义菜单），让 AI / CI 可以不触发 `EditorUtility.DisplayDialog` 就运行。**下一步衔接**：L1-4（5 处 Prism `_family` 字段修复）是批次 2 的最后未闭合项，按 Plan §三原文需要 owner 逐项确认目标值后独立执行。建议的下一轮输入格式："L1-4 的 5 个 Prism 目标值分别是：ShebaP_Homing → Rheology、ShebaP_Boomerang → 保持 Rheology（或改为 Anomaly）、ShebaP_Bounce → Rheology、ShebaP_MinePlacer → Fractal（或改为 Anomaly）、第 5 个待定 → XXX"——只要 owner 给出这 5 个决策，L1-4 可以作为批次 2 的最终收尾 commit 一次性执行。
