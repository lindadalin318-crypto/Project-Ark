...
function createTopBody(isPlayer, rpm, launchAngle, launchPower, spinDir) {
  ...
  return {
    isPlayer,
    x: isPlayer ? ARENA.cx - 80 : ARENA.cx + 80,
    y: isPlayer ? ARENA.cy + 60 : ARENA.cy - 60,
...
function physicsStep(top, dt) {
  ...
  top.rpm = Math.max(0, top.rpm - decayRate);
...
function checkTopCollision(p, e) {
  ...
  if (dist < ARENA.collisionDist) {
    ...
    if (impact < 0) { // approaching
      ...
      p.rpm = Math.max(0, p.rpm - enemyArDmg * spinRpmDrainMult);
      e.rpm = Math.max(0, e.rpm - arDmg * spinRpmDrainMult);

      p.hitFlash = 8;
      e.hitFlash = 8;

      // Separate tops
      const overlap = ARENA.collisionDist - dist;
      p.x += nx * overlap * 0.5;
      p.y += ny * overlap * 0.5;
      e.x -= nx * overlap * 0.5;
      e.y -= ny * overlap * 0.5;
...
function renderArena() {
  ...
  // ---- Bowl background ----
  const grad = ctx.createRadialGradient(ARENA.cx, ARENA.cy, 0, ARENA.cx, ARENA.cy, ARENA.radius);
  ...
  ctx.beginPath();
  ctx.arc(ARENA.cx, ARENA.cy, ARENA.radius * 0.6, 0, Math.PI * 2);
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(ARENA.cx - 20, ARENA.cy); ctx.lineTo(ARENA.cx + 20, ARENA.cy);
  ctx.moveTo(ARENA.cx, ARENA.cy - 20); ctx.lineTo(ARENA.cx, ARENA.cy + 20);
  ctx.stroke();
...
function drawTop(ctx, top, colorMain, colorAccent) {
  ...
  const cx = top.x + wobX, cy = top.y + wobY;
  const r = top.radius;
