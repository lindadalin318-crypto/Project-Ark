...
$(window).on("load", function() {
    ...
    // Reusable toggle for the arrow and its first child list
  ...
  const noLinkSpanHandler = () => {
    var $arrow = $(this).prev('.arrow');
    if ($arrow.length) toggleArrowAndList($arrow);
  }

  // Clicks on "no-link" spans should toggle the preceding arrow
  $noLinkSpans.on('click', noLinkSpanHandler);

  // Keyboard on "no-link" spans (Space/Enter) should also toggle the preceding arrow
