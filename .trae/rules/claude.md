...
| **URP Bloom / 后处理效果不可见** | Camera 的 `UniversalAdditionalCameraData.renderPostProcessing` 默认关闭，Volume Profile 中的 Bloom/Vignette/DoF 等效果全部不生效，但粒子系统等非后处理 VFX 正常——症状表现为"部分 VFX 正常、Bloom 不工作"，极易误判为代码 bug | **VFX 不可见时，第一步查 Camera 的 Post Processing 开关**（Inspector > Camera > Rendering > Post Processing），确认勾选后再排查代码逻辑。2D 项目中此开关容易被遗漏因为默认关闭。**额外注意**：开启后 Volume Profile 中所有 `active: 1` 的效果会同时生效，需审查是否有不需要的效果（如 DepthOfField）被连带激活 |
