...
| Coordinator 依赖接线 | `SpaceLifeSetupWindow` 自动回填 | 手工拖线 `DialogueDatabase` + `DialogueServiceRouter`，Presenter 由 `ServiceLocator` 自动解析（详见 §12.4 / §15.3） |
...
> 本章沉淀自 `Docs/2_TechnicalDesign/Combat/StarChart_Governance_Plan.md`（立规计划）与 Step 1-2 的现役主链扫码、资产映射扫码结果。
>
> **首要目的**：在下一轮批量生产新武器（新 Core / Prism / Sail / Satellite）之前，把"施工必须怎么做"钉死，让新武器开发按规施工而非重新考古。
>
> **联动真相源**：
>
> - 现役链路真相：`Docs/2_TechnicalDesign/Combat/StarChart_CanonicalSpec.md`
> - 资产 owner 映射：`Docs/2_TechnicalDesign/Combat/StarChart_AssetRegistry.md`
>
> **冲突裁决**：本章只写"施工应该怎么做"。现役链路 / owner / 路径定义以 `CanonicalSpec` 和 `AssetRegistry` 为准。三者冲突时以 `CanonicalSpec` 和 `AssetRegistry` 为先。

### 13.1 模块边界

| 范围 | 路径 |
|---|---|
| **Runtime** | `Assets/Scripts/Combat/StarChart/` + `Assets/Scripts/Combat/Projectile/` + `Assets/Scripts/UI/StarChart/` |
| **Editor** | `Assets/Scripts/Combat/Editor/`（`Batch5AssetCreator.cs` / `ShebaAssetCreator.cs` / `ShapeContractValidator.cs` 等） |
| **Data** | `Assets/_Data/StarChart/` 全部子目录（`Cores/` / `Prisms/` / `Sails/` / `Satellites/` / `Prefabs/`） + `PlayerInventory.asset` |
| **Behavior Prefab** | `Assets/_Prefabs/StarChart/`（Sail / Satellite 的行为 Prefab） |

**关联文档三件套**：

- GDD：`Docs/1_GameDesign/` 下对应章节（以实际存在的为准）
- 现役链路真相：`Docs/2_TechnicalDesign/Combat/StarChart_CanonicalSpec.md`
- 资产 owner 映射：`Docs/2_TechnicalDesign/Combat/StarChart_AssetRegistry.md`

**关联横向规范**：`ProceduralPresentation_WorkflowSpec.md`（preview 工具边界，特别是 EchoWaveProceduralPreviewMenu）。

**与其他章节的分工**：

- 投射物 Layer / 碰撞矩阵 / LayerMask 相关规则 → 见 **Section 11 Combat / Projectile 模块规则**（本章不重复）
- 程序化 Texture 预览物（如 EchoWave 波环）alpha 验证 → 见 **Section 11.4.2**（本章不重复）
- 本章只管 StarChart 特有的施工规则（SO / Prefab / Modifier 注入 / Inventory 一致性 / 对象池复位）

### 13.2 模块目标

- 让**批量生产新武器**的施工成本可预测：查 Spec → 查 Registry → 按 Rules 施工 → 走验收清单
- 让**对象池防御性复位**成为肌肉记忆而非考古结果（历史上 LaserBeam 颜色泄漏 / Modifier 组件累积 / Projectile 缩放残留都来自复位遗漏）
- 让 **Modifier 注入路径**（Tint 走 `TintModifierPrefabs`，Anomaly 走 `AnomalyModifierPrefab`）在编码时一目了然，不被历史实现的"两条路径"混淆
- 让 **PlayerInventory 一致性约束**在新增资产时被主动维护，而不是靠运行时 dangling 报错才发现

### 13.3 实现规则

#### 13.3.1 新武器施工八条清单（每次新 Core / Prism / Sail / Sat 必须全过）

以下 8 条对应 §13.5 验收清单 8 条，一一对齐。

1. **SO 资产创建走 AssetCreator**：扩展或新建 Editor AssetCreator 工具（参考 `ShebaAssetCreator` 结构），禁止直接在 Project 窗口右键创建 `.asset`——右键创建不会写入 `PlayerInventory.asset._ownedItems`，会导致运行时 lookup 失败。
2. **DisplayName 全局唯一**：`_displayName` 在所有 StarCore / Prism / Sail / Satellite 中必须全局唯一。Save / Load 与 Inventory 都走 name lookup，冲突会导致静默取错（见 §13.4 第 4 条）。现存冲突风险见 `StarChart_AssetRegistry.md` 第 2 节。
3. **Tint Prism 必须配套 Modifier Prefab**：挂 `IProjectileModifier` 组件的 Prefab，路径 `Assets/_Data/StarChart/Prefabs/`。通过 `PrismSO._projectileModifierPrefab` 字段引用。
4. **Anomaly Core 的 Modifier 走专属字段**：Anomaly Core 的 Modifier 通过 `StarCoreSO._anomalyModifierPrefab` 字段引用，由 `SnapshotBuilder` 写入 `CoreSnapshot.AnomalyModifierPrefab`，仅 `SpawnAnomalyEntity` 额外注入。**禁止**把 Anomaly Modifier 塞进 `PrismSO._projectileModifierPrefab`，也**禁止**和 Tint 路径混用（见 §13.4 第 2 条）。
5. **新 Projectile Prefab 必须 IPoolable + 预热**：挂 `IPoolable` 实现，注册到 `PoolManager` 预热配额。Anomaly 家族当前复用 Matter 的 `Projectile_Matter.prefab`（见 `AssetRegistry` 第 2 节），新增专属 Anomaly Projectile 必须同步更新 `AssetRegistry` Prefab 表。
6. **OnReturnToPool 按五项全检**：运行时字段 / event 订阅 / 动态添加的 Component / Transform（scale / position / rotation）/ 视觉状态（颜色 / alpha / Trail）。遵循 `CLAUDE.md` 的"对象池回收清单"。新 Projectile / Modifier Prefab 必须补齐五项复位。
7. **新 SO 必须追加到 PlayerInventory**：`PlayerInventory.asset._ownedItems` 数组。AssetCreator 应自动完成此步；若手工介入，必须在同一次提交内补完，禁止"先加 SO 再补 Inventory"的两阶段提交。
8. **新 Shape 必须过 ShapeContractValidator**：通过 Editor 菜单触发 `ShapeContractValidator` 校验，确认 `_shape` 字段与 `StarChartEnums.ItemShape` 枚举对齐、占位关系正确。

#### 13.3.2 禁止事项（硬约束）

- **禁止在 Runtime 中修改 `StarChartItemSO` 字段**：违反"运行时数据隔离"（`CLAUDE.md` 架构原则第 6 条）。运行时需要变化的属性走 runtime 副本字段或 `CoreSnapshot` / `FiringSnapshot`。
- **禁止在 `SpawnProjectile` 的 switch 之外判断 `CoreFamily`**：`ProjectileSpawner.SpawnProjectile()` 的 `switch(CoreFamily)` 是分发唯一权威（见 `CanonicalSpec` 第 3 节）。`StarChartController.ExecuteFire` 会委托到 `ProjectileSpawner`；其他位置判断家族会导致分发逻辑被割裂到多处，新增家族时极易漏改。
- **禁止绕过 `SnapshotBuilder` 直接构造 `CoreSnapshot` / `FiringSnapshot`**：`SnapshotBuilder.Build()` 是构建发射快照的唯一权威路径。绕过它会导致 Stat 聚合、Modifier 收集、Tint/Anomaly 分路等逻辑不一致。
- **禁止手工在 Project 窗口创建 SO 资产**：必须走 AssetCreator。例外名单仅限 `StarChart_AssetRegistry.md` 第 9 节的 `Unknown-Historical` 条目，且禁止新增。
- **禁止在单个 Modifier Prefab 上挂多个 `IProjectileModifier` 而不评估相互影响**：多个 Modifier 共存时的聚合顺序、相互覆盖关系必须在 PR 描述中显式说明。
- **禁止手工修改 `PlayerInventory.asset._ownedItems`**：应通过 AssetCreator 自动追加。若必须手工改（例如 Unknown-Historical 资产补回），必须同时验证 GUID 存在对应 SO 资产。
- **禁止修改 Legacy AssetCreator**：`Batch5AssetCreator` 和 `ShebaAssetCreator` 两个工具目前均标注为 Legacy / Hidden from menu（见 `AssetRegistry` 第 4-5 节）。新批次武器必须新建 AssetCreator 工具，不在 Legacy 工具上叠加。

#### 13.3.3 Debug / Preview 工具约束

- `EchoWaveProceduralPreviewMenu` 属于 ProceduralPresentation 体系，只允许 preview，**不得**在 Runtime 接管 EchoWave 生成链。
- 未来新增 StarChart 相关 preview 工具必须显式与正式链路隔离：
  - 只读 SO，不写回资产
  - 不在 Play Mode 下持续 override 正式运行态
  - 不替换正式 renderer 或正式 SpawnProjectile 分发
- 参考：`ProceduralPresentation_WorkflowSpec.md` 第 3 / 7 节。
- 与 `Implement_rules.md` §3.4（Ship/VFX 的 Debug 不得接管正式运行链）同源：**Debug 工具只能观察，不能成为行为参与者。**

#### 13.3.4 Modifier 注入路径的两条清晰分路

为避免实现时混淆（历史上曾误把 Anomaly 的 Modifier 塞进 Tint 路径），本节单独把两条分路固化：

| 分路 | SO 字段 | Snapshot 字段 | 注入点 | 适用家族 |
|---|---|---|---|---|
| **Tint 路径** | `PrismSO._projectileModifierPrefab` | `CoreSnapshot.TintModifierPrefabs`（List） | `SnapshotBuilder.Build()` 统一收集，所有家族走 `InstantiateModifiers` | 所有 Tint Prism（目前仅 `TintPrism_FrostSlow` 一个）+ 其余 Sheba Prism |
| **Anomaly 路径** | `StarCoreSO._anomalyModifierPrefab` | `CoreSnapshot.AnomalyModifierPrefab`（单个） | `SpawnAnomalyEntity` 额外注入 | 仅 Anomaly 家族 Core |

**施工纪律**：

- 新 Tint Prism → 只写 `PrismSO._projectileModifierPrefab`，不管 Anomaly 字段
- 新 Anomaly Core → 只写 `StarCoreSO._anomalyModifierPrefab`，不借用 Prism 字段
- 若某天真的需要"Anomaly Core 配带 Tint Prism 的组合效果"——Tint 路径已经覆盖，不需要额外机制，因为 Tint 是 Prism 层而非 Core 层

### 13.4 踩坑总结

#### 13.4.1 对象池状态泄漏三连坑

- **现象**：回收的 Projectile / LaserBeam 再次出池后颜色不对、Modifier 组件累积、Transform scale 未复位。
- **根因**：`OnReturnToPool()` 遗漏某些字段的重置。
- **防御**：遵循"对象池回收清单"五项全检（见 `CLAUDE.md` 架构原则第 5 条 + §13.3.1 第 6 条）。新 Projectile / Modifier Prefab 评审时把"五项是否全到位"作为硬验收项。

#### 13.4.2 Tint vs Anomaly 的 Modifier 注入路径混淆

- **现象**：新 Anomaly Core 的 Modifier 被塞进 `PrismSO._projectileModifierPrefab`，导致 Modifier 在 Tint 路径被 `InstantiateModifiers` 收集，但 Anomaly 专属的 `SpawnAnomalyEntity` 注入路径拿到 null。
- **根因**：两条路径在历史实现中同时存在，字段名相似，新人不看 `SnapshotBuilder` 源码容易弄反。
- **防御**：`AssetRegistry` 第 2 节有现役引用表作交叉验证；§13.3.4 两条分路固化在本章节。新 Core/Prism 评审时必查 SO 字段写入的是哪条分路。

#### 13.4.3 `StatModifier` Additive / Multiplicative 聚合链的初始化边缘情况

- **现象**：首次装备 / 首次发射时 Stat 聚合结果与预期不一致，随后恢复正常。
- **根因**：`StatModifier` 链在 Snapshot 构建阶段的初始化顺序未明确覆盖所有边缘（例如空列表、单元素列表、multiplier = 0 等）。
- **防御**：新增 Stat 聚合逻辑必须同步写 `StarChart.Tests` 下的单元测试覆盖三类边缘：空列表 / 单元素 / multiplier 为 0。

#### 13.4.4 DisplayName 冲突导致 Inventory / Save lookup 静默取错

- **现象**：Inventory 中装备了 A，Save / Load 后装备变成了 B。
- **根因**：`_displayName` 在多个 SO 中重复，Inventory / Save 的 `Dictionary<string, SO>` 后写覆盖先写。
- **防御**：§13.3.1 第 2 条；`AssetRegistry` 第 2 节固化了当前两对"同名但类型不同当前不冲突"的风险条目（`FractalPrism_TwinSplit` vs `ShebaP_TwinSplit` 都叫 "Twin Split"；`AnomalyCore_Boomerang` 与 `ShebaP_Boomerang` 都叫 "Boomerang"）。新 SO 必须 grep 一遍确认不撞名。

#### 13.4.5 PlayerInventory dangling reference（历史疑云）

- **现象**：曾怀疑 Inventory 的 21 个 GUID 存在 dangling（指向不存在的 SO）。
- **根因**：Step 2 立规扫码阶段交叉验证确认当前 **21 个 GUID 全部对应现存 SO**，无 dangling。
- **防御**：§13.3.1 第 7 条约束新增 SO 同步追加 Inventory；未来若出现删除 SO 的需求，必须在同一提交内从 Inventory 移除 GUID，否则即引入 dangling。

#### 13.4.6 新 Core 忘记在 `SpawnProjectile` 的 switch 加分支

- **现象**：新 Core 配置了 Family = 新家族，运行时发射后 Projectile 行为表现为 Matter 家族。
- **根因**：`ProjectileSpawner.SpawnProjectile()` 的 `switch(CoreFamily)` 缺失分支，`default` fallback 到 Matter 处理，静默错误。
- **防御**：§13.3.2 明确 switch 是分发唯一权威；新增 CoreFamily 时把"补 switch 分支"列为 PR 强制 checklist 一项。考虑未来在 `default` 中加 `Debug.LogError` 把 silent no-op 转成响亮失败。

#### 13.4.7 Anomaly 家族复用 Matter Projectile 的隐性假设

- **现象**：Anomaly Core（如 `AnomalyCore_Boomerang`）不挂自己的 Projectile Prefab，而是复用 `Projectile_Matter.prefab`。这是现役有意设计，但新人易误判为"漏配"。
- **根因**：当前 Anomaly 的行为差异由 `Modifier_Boomerang`（挂 `BoomerangModifier`）承担，Projectile 本身不需要区别。见 `AssetRegistry` 第 2 节。
- **防御**：`AssetRegistry` Prefab 表显式记录此复用关系。新 Anomaly Core 评审时必须回答：本次新增是否仍复用 `Projectile_Matter.prefab`？若否，必须同步创建专属 Projectile Prefab 并更新 `AssetRegistry`。

#### 13.4.8 PlayerInventory 一致性约束 = AssetCreator 权威化

- **现象**：历史上曾出现"手工创建了 SO 资产但没追加到 PlayerInventory"的遗留，典型例子是 `TestSpeedSail.asset`（见 `AssetRegistry` 第 9 节 Unknown-Historical）。
- **根因**：手工右键创建 SO 时绕过了 AssetCreator 自动追加 Inventory 的流程。
- **防御**：§13.3.1 第 1 / 7 条（强制走 AssetCreator，Inventory 作为一致性约束）+ §13.3.2 禁止手工创建 + §13.3.2 禁止修改 Legacy AssetCreator（新批次必须新建）。

### 13.5 验收清单

新武器 / 新部件上线前必过 8 条（对应 §13.3.1 八条施工清单一一对齐）：

1. **SO 创建走了 AssetCreator？**（禁止手工右键创建）
2. **DisplayName 未冲突？**（grep 全项目 `_displayName` 确认）
3. **Shape 过了 ShapeContractValidator？**（Editor 菜单触发）
4. **Projectile Prefab 有 IPoolable 且完整五项复位？**（运行时字段 / event / 动态 Component / Transform / 视觉状态）
5. **Modifier 注入路径正确？**（Tint → `PrismSO._projectileModifierPrefab`；Anomaly → `StarCoreSO._anomalyModifierPrefab`；不混用）
6. **Inventory 已追加？**（`PlayerInventory.asset._ownedItems` 含新 SO 的 GUID）
7. **Play Mode 实测四步无残留？**（装备 → 发射 → 回收 → 再装备）
8. **若是新 CoreFamily，`ProjectileSpawner.SpawnProjectile` 的 switch 已补分支？**（否则写一条 `ImplementationLog` 显式标注）
...
```
1. 查 Spec       → Docs/2_TechnicalDesign/Combat/StarChart_CanonicalSpec.md（现役链路、owner、依赖反转边界）
2. 查 Registry   → Docs/2_TechnicalDesign/Combat/StarChart_AssetRegistry.md（对照现役资产、owner、Prefab 复用关系）
3. 按 Rules 施工 → 本章 §13.3（新武器八条施工清单 + 禁止事项 + Modifier 两条分路）
4. 跑验收清单    → 本章 §13.5（八条验收）
5. 更新 Registry → 新 SO / Prefab 同步追加到 AssetRegistry 对应表
6. 补 ImplementationLog → 遵循 CLAUDE.md 第 7 步强制规则
```

#### 难定位 Bug

```
1. 先查 Registry  → 确认资产完整（Inventory 是否 dangling、SO 字段是否正确）
2. 再查 Spec      → 确认是否走对链路（switch 分支、SnapshotBuilder 写入路径）
3. 最后才 trace Runtime → StarChartController / WeaponTrack / SnapshotBuilder
4. 若根因是模块边界模糊，优先补 §13.3 规则 / validator，而不是继续叠运行时保护代码
```
...
- **StarChart**：见 Section 13（2026-04-24 立规计划 Step 3 沉淀，含八条施工清单 + Modifier 双路径分路 + AssetRegistry 联动，配套真相源 `Docs/2_TechnicalDesign/Combat/StarChart_CanonicalSpec.md` 与 `StarChart_AssetRegistry.md`）
