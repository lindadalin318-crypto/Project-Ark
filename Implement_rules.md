...
- **禁止在 `SpawnProjectile` 的 switch 之外判断 `CoreFamily`**：`StarChartController.SpawnProjectile()` 的 `switch(CoreFamily)` 是分发唯一权威（见 `CanonicalSpec` 第 3 节）。其他位置判断家族会导致分发逻辑被割裂到多处，新增家族时极易漏改。
...
- **根因**：`StarChartController.SpawnProjectile()` 的 `switch(CoreFamily)` 缺失分支，`default` fallback 到 Matter 处理，静默错误。
...
8. **若是新 CoreFamily，`SpawnProjectile` 的 switch 已补分支？**（否则写一条 `ImplementationLog` 显式标注）
